...
Mobile.callTestCase(findTestCase('VBL_Mobile/Re-Usables/Web Login and Navigation/Navigate to Focus Brand'), [:], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Code.size(); i++) {
    if (Code.get(i) != 'NULL') {
        ...
        if (WebUI.verifyElementPresent(findTestObject('XXXXXXXXXXXX/Web Part/FocusBrand(MSL)/td_Tag(GridCode)'), 2, FailureHandling.OPTIONAL)) {
            ...
            if (WebUI.verifyElementPresent(findTestObject('XXXXXXXXXXXX/Web Part/FocusBrand(MSL)/td_Tag(GridCode)'), 2, FailureHandling.OPTIONAL)) {
                'Focusbrand group name Aleardy exist!'
                WebUI.takeScreenshot()
                ...
                'Focusbrand group name Not Aleardy exist!'
                WebUI.takeScreenshot()
                ...
                for (int c = 0; c < SkuName.size(); c++) {
                    ...
                    GlobalVariable.label2 = Sequence.get(c)

                    WebUI.setText(findTestObject('XXXXXXXXXXXX/Web Part/FocusBrand(MSL)/SkuNameFilter'), SkuName.get(c))
            ...
            for (int d = 0; d < LocationName.size(); d++) {
                if (LocationName.get(d) != 'NULL') {
                    ...
                    if (checkbox_type == unchecked_checkbox) {
                        'Entered location field exist but check box not enable so we need to enable it.'
                        WebUI.takeScreenshot()
            ...
            WebUI.verifyElementPresent(findTestObject('XXXXXXXXXXXX/Web Part/FocusBrand(MSL)/Focus-UpdatedSuccessful_Alert'), 
                5)
            ...
            'Focus brand name not exist so we need to create'
            WebUI.takeScreenshot()
            ...
            if (WebUI.verifyElementPresent(findTestObject('XXXXXXXXXXXX/Web Part/FocusBrand(MSL)/td_Tag(GridCode)'), 2, FailureHandling.OPTIONAL)) {
                ...
                'Focus brand group name not exist so we need to create'
                WebUI.takeScreenshot()
            ...
            WebUI.verifyElementPresent(findTestObject('XXXXXXXXXXXX/Web Part/FocusBrand(MSL)/Focus-SavedSuccessful_Alert'), 
                5)
...
not_run: WebUI.callTestCase(findTestCase('VBL_Mobile/Re-Usables/Web Login and Navigation/ClearCashe Login'), [:], FailureHandling.STOP_ON_FAILURE)
