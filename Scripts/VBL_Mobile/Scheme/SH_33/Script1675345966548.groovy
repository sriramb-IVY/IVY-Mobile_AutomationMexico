...
List<Double> BUYandGET_SKU01_qty = new ArrayList<String>()

//List<Int> BUYandGET_SKU01_qty = new ArrayList<String>()
for (int i = 0; i < Retailer.size(); i++) {
    if (Retailer.get(i) != 'null') {
		...
		BUYandGET_SKU01_qty.add(Integer.parseInt(Slab_1_Min_Qty))
        ...
        if ((RetailerApplyType == GlobalVariable.Variable) || (RetailerApplyType == GlobalVariable.Fixed_DD)) {
            ...
            GlobalVariable.Scheme_Free_SKU = findTestData('VBL_Mobile Input Data/Fixed_Variable_Scheme').getValue('FreeProduct_Slab1', Scheme_Index)
            ...
            Mobile.verifyMatch(FreeSKU_Qty_Slab1, findTestData('VBL_Mobile Input Data/Fixed_Variable_Scheme').getValue('FreeProduct_Case_Slab1', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            BUYandGET_SKU01_qty.add(Integer.parseInt(FreeSKU_Qty_Slab1))

            KeywordUtil.logInfo(FreeSKU_Qty_Slab1 + ' :Slab 1 Free Product Case Qty correctly displayed !')

            Expecetd_FreeSKU_MinQty_Slab1 = ('Min:' + findTestData('VBL_Mobile Input Data/Fixed_Variable_Scheme').getValue('FreeProduct_Case_Slab1', Scheme_Index))
            ...
            Expecetd_FreeSKU_MaxQty_Slab1 = ('Max:' + findTestData('VBL_Mobile Input Data/Fixed_Variable_Scheme').getValue('FreeProduct_Case_Slab1_MAX', Scheme_Index))
            ...
            Mobile.verifyMatch(Slab1_FreeSku_CaseQty, findTestData('VBL_Mobile Input Data/Fixed_Variable_Scheme').getValue('FreeProduct_Case_Slab1', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            KeywordUtil.logInfo(Total.toString() + ' : Slab_1 Calculated 0rder amt correctly displayed !')
	...
	for (int j = 0; j < BUYandGET_SKU01_qty.size(); j++) {
		sum_1 += BUYandGET_SKU01_qty.get(j)
	...
	//SKU_Conversion_Qty = findTestData('VBL_Mobile Input Data/Scheme_Stock_Validation').getValue('SKU_Conversion_Qty', 3)
	...
	ExcelKeywords.setValueToCellByIndex(sheet1, 3, 5, SIH_BUYandGET_SKU01)
