...
for (int i = 0; i < Retailer.size(); i++) {
    if (Retailer.get(i) != 'null') {
        ...
        if ((RetailerApplyType == GlobalVariable.Variable) || (RetailerApplyType == GlobalVariable.Fixed_DD)) {
            ...
            CheckBox = Mobile.getAttribute(findTestObject('XXXXXXXXXXXX/Mobile Part/Store_Actvy/OrderInvoice/Scheme/Scheme_Percentage_CheckBox'), 'checked', 1, FailureHandling.STOP_ON_FAILURE)
            ...
            //			Total_MRP = ((Double.parseDouble(Actual_CaseSize) * Double.parseDouble(Slab_1_Min_Qty)) * Double.parseDouble(
            //				Actual_MRP))
