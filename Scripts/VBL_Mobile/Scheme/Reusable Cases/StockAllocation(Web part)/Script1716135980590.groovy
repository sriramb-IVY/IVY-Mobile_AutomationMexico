...
WebUI.callTestCase(findTestCase('VBL_Mobile/Re-Usables/Web Login and Navigation/Navigate to StockAdjustmentCreate'), [:], FailureHandling.STOP_ON_FAILURE)
...
sheet_name = 'Stock allocation'

 file_name = 'Web Input Data'

ArrayList<String> Store = CustomKeywords.'poi.VBL.Automation.GetAllWebDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'Store')

ArrayList<String> User_Details = CustomKeywords.'poi.VBL.Automation.GetAllWebDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'User_Details')
...
for (int i = 0; i < Warehouse.size(); i++) {
    if (Warehouse.get(i) != 'NULL') {
        ...
        for (j = 0; j < SKU_Name.size(); j++) {
            ...
            WebUI.verifyElementPresent(findTestObject('XXXXXXXXXXXX/Web Part/Stock Allocation/Global Variables/EnterCaseValue_Td Tag'), 2, FailureHandling.OPTIONAL)

            GlobalVariable.CaseQty = '50'
