...
queryString = (('select * from AppData_Attendance_Time_Detail where aatd_su_id= \'' + GlobalVariable.vanseller_user_id + '\' and AATD_Date = \'' + GlobalVariable.label) + '\'')
...
while (recordSet.next()) {
	String data1 = recordSet.getObject('AATD_Date')

	KeywordUtil.logInfo(data1)

	String data = recordSet.getObject('aatd_su_id')
 
	KeywordUtil.logInfo(data)

queryString01 = (('delete from AppData_Attendance_Time_Detail where aatd_su_id= \'' + GlobalVariable.vanseller_user_id + '\' and AATD_Date = \'' + GlobalVariable.label) + '\'')
