...
queryString = ((('select * from appdata_collection_request where cr_sih_no= \'' + GlobalVariable.Transaction_No) + '\' and CR_Receipt_Date =\'' + DB_Currentdate) + '\'  ')
...
while (recordSet.next()) {
	String data1 = recordSet.getObject('CR_SIH_No')

	KeywordUtil.logInfo(data1)

	GlobalVariable.label = data1
	
	String data = recordSet.getObject('CR_Receipt_Date')
 
	KeywordUtil.logInfo(data)

queryString01 = (('delete from appdata_collection where c_sih_no=\'' + GlobalVariable.label) + '\'')
...
queryString0 = ((('delete from appdata_collection_request where cr_sih_no= \'' + GlobalVariable.Transaction_No) + '\' and CR_Receipt_Date =\'' + DB_Currentdate) + '\'  ')
...
queryString1 = (('delete from appdata_pending_bills where PB_UserId =  \'' + GlobalVariable.vanseller_user_id + '\' and PB_SIH_Date =\'' + DB_Currentdate) + '\' ')
