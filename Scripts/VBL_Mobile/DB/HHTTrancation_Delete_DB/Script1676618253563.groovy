...
queryString = ('Select * from HHTTransactionDataQueueList where UserId =  \'' + GlobalVariable.vanseller_user_id + '\' order by 1 desc')
...
while (recordSet.next()) {
	String data1 = recordSet.getObject('TransactionId')

	KeywordUtil.logInfo(data1)

	GlobalVariable.label = data1
	
	String data2 = recordSet.getObject('UserId')
	
	println(data2)	


queryString01 = (('delete from HHTTransactionDataQueueList_Details where UserId =  \'' + GlobalVariable.vanseller_user_id + '\' and TransactionId = \'' + GlobalVariable.label) + '\'')
...
queryString0 = (('delete from HHTTransactionDataQueueList where UserId =  \'' + GlobalVariable.vanseller_user_id + '\' and TransactionId = \'' + GlobalVariable.label) + '\'')
