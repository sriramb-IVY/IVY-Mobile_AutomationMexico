...
queryString = (('select * from AppData_Digital_Content_Tracking_Header where dcth_user_id= \'' + GlobalVariable.vanseller_user_id + '\'  and DCTH_Date =\'' + DB_Currentdate) + '\'')
...
while (recordSet.next()) {
	String data1 = recordSet.getObject('DCTH_Id')

	KeywordUtil.logInfo(data1)

	GlobalVariable.label = data1
	
	String data = recordSet.getObject('DCTH_Date')
 
	KeywordUtil.logInfo(data)

queryString01 = (('delete from AppData_Digital_Content_Tracking_Detail where dctd_dcth_id=\'' + GlobalVariable.label) + '\' ')
...
queryString0 = (('delete from AppData_Digital_Content_Tracking_Header where dcth_user_id= \'' + GlobalVariable.vanseller_user_id + '\'  and DCTH_Id =\'' + GlobalVariable.label) + '\' ')
