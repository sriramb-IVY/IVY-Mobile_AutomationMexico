...
for (def index : (0..Nav.size() - 1)) {
    ...
    if ((Nav[index]) == (Nav[previousIndex])) {
        ...
        List<WebElement> NavLinks = WebUiCommonHelper.findWebElements(findTestObject('PNG Product/common object/a_NavigationLink'), 
            5)
        ...
        if (NavLinks.size() > 0) {
            ...
            WebUI.scrollToElement(findTestObject('PNG Product/DatePicker_and _Navigation/Navigation/Navigation'), 30)
    ...
    if ((Nav[index]) == 'Receivables') {
		WebUI.scrollToElement(findTestObject('Surendran/IvySalesOrder/Page_IvyDMS/a_Receivables'),30)
