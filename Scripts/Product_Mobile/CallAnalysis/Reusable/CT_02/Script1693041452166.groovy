...
not_run: WebUI.callTestCase(findTestCase('Product_Mobile/Common/Web Login and Navigation/Navigate to StockAdjustmentCreate'), 
    [:], FailureHandling.STOP_ON_FAILURE)

not_run: if (WebUI.waitForElementNotVisible(findTestObject('Web Part/Stock Allocation/Button/Freeze_btn'), 
...
for (int i = 0; i < Warehouse.size(); i++) {
    if (Warehouse.get(i) != 'NULL') {
		...
		WebUI.scrollToElement(findTestObject('Web Part/Stock Allocation/Global Variables/UserSelect_Span b Tag'), 2, FailureHandling.OPTIONAL)
