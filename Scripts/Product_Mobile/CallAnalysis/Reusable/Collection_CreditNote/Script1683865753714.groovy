...
//GlobalVariable.Transaction_No = '29711222022143336'
...
queryString4 = (('select top 1 * from appdata_collection where c_sih_no=\'' + GlobalVariable.Transaction_No) + '\' and C_ReferenceNo like \'' + CR + '\' order by 1 desc')

//queryString4 = ('select top 1 * from appdata_collection where  C_ReferenceNo like \'' + CR + '\' order by 1 desc')
...
if (recordSet4.next()) {
    ...
    KeywordUtil.logInfo(GlobalVariable.label + ' : is the receipt number in the DataBase is reflected in the Appliction')
    ...
    'Reference no'
//    GlobalVariable.label = findTestData('Mobile Input Data/Collection').getValue('TransactionNo', 1)
//
...
//
//    KeywordUtil.logInfo('Reference no displayed properly in database and collection view Screen')
    ...
    KeywordUtil.logInfo(GlobalVariable.Collection_Amt + '  : Paid Amount is Displayed properly in database and collection view Screen')
...
WebUI.verifyMatch(paymentype,  findTestData('Mobile Input Data/Collection').getValue('Mode_of_Collection_3', 1), false, FailureHandling.STOP_ON_FAILURE)
...
if (recordSet5.next()) {
    ...
    KeywordUtil.logInfo(GlobalVariable.label1 + ' : The Payment Type displayed properly in database and collection view Screen')
