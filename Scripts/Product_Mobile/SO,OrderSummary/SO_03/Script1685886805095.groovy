...
Mobile.callTestCase(findTestCase('Product_Mobile/Common/Web Login and Navigation/Navigate to Focus Brand (MSL)'), [:], FailureHandling.STOP_ON_FAILURE)
...
ArrayList<String> GroupCode = CustomKeywords.'poi.Automation.GetAllWebDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'GroupCode')

ArrayList<String> GroupDesc = CustomKeywords.'poi.Automation.GetAllWebDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'GroupDesc')

ArrayList<String> SkuName = CustomKeywords.'poi.Automation.GetAllWebDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'SkuName')

ArrayList<String> LocationName = CustomKeywords.'poi.Automation.GetAllWebDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'LocationName')
...
for (int i = 0; i < Code.size(); i++) {
    if (Code.get(i) != 'NULL') {
        ...
        if (WebUI.verifyElementPresent(findTestObject('Web Part/FocusBrand(MSL)/td_Tag(GridCode)'), 2, FailureHandling.OPTIONAL)) {
            ...
            if (WebUI.verifyElementPresent(findTestObject('Web Part/FocusBrand(MSL)/td_Tag(GridCode)'), 2, FailureHandling.OPTIONAL)) {
                ...
                for (int b = 0; b < SkuName.size(); b++) {
                    ...
                    checkbox_type = WebUI.getAttribute(findTestObject('Web Part/FocusBrand(MSL)/SkuCheckBox_td_Tag(Global)'), 'src', FailureHandling.STOP_ON_FAILURE)
            ...
            WebUI.verifyElementPresent(findTestObject('Web Part/FocusBrand(MSL)/UpdatedSuccessful_PopUp'), 5)
            ...
            WebUI.callTestCase(findTestCase('Product_Mobile/Common/DatePicker_and _Navigation/Date Picker'), [('DateValue') : 'Today'], FailureHandling.STOP_ON_FAILURE)

            WebUI.click(findTestObject('Web Part/FocusBrand(MSL)/EffectiveTo'))

            not_run: WebUI.delay(5)

            WebUI.callTestCase(findTestCase('Product_Mobile/Common/DatePicker_and _Navigation/Date Picker - Future(1year)'), [('DateValue') : 'tomorrow'], FailureHandling.STOP_ON_FAILURE)
            ...
            WebUI.verifyElementPresent(findTestObject('Web Part/FocusBrand(MSL)/SaveSuccessful_PopUp'), 5)
        ...
        println(' To verify user can able to create mustsell')
    }
}
