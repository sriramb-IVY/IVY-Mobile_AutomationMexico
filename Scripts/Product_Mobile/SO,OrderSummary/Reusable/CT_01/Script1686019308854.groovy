...
for (int i = 0; i < Warehouse.size(); i++) {
    if (Warehouse.get(i) != 'NULL') {
		...
		WebUI.scrollToElement(findTestObject('Web Part/Stock Allocation/Drop down values/Store'), 0, FailureHandling.OPTIONAL)
        ...
        for (j = 0; j < SKU_Name.size(); j++) {
			...
			WebUI.scrollToElement(findTestObject('Web Part/Stock Allocation/Filter/Appy_Btn'), 0, FailureHandling.OPTIONAL)
...
while (recordSet2.next()) {
    ...
    println('Total piece Qty  :  ' + GlobalVariable.PieceQty )///VL_02 To verify DB for geting vanload n0)
