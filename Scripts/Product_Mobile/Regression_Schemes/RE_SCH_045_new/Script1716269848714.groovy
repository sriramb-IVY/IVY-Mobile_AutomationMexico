...
String sheet_name = 'B9_MTR_DV_2S_Buy_Excl_SL'
...
for (int i = 0; i < Retailer.size(); i++) {
    KeywordUtil.logInfo(Quantity_variations.get(i))
}

Mobile.tap(findTestObject('Mobile/Common/Btn_Menu'), 10)

WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/B9_MTR_DV_2S_Buy_Excl_SL').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    KeywordUtil.logInfo('Scheme_Index' + Scheme_Index.toString())
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/B9_MTR_DV_2S_Buy_Excl_SL').getValue('SKU_Name', Scheme_Index)
    ...
    Min_Qty_1 = findTestData('Batch_Scheme_Inputs_01/B9_MTR_DV_2S_Buy_Excl_SL').getValue('Buy_Quantity', Scheme_Index)
    ...
    KeywordUtil.logInfo(Total.toString() + ' : Scheme Total Amount calculated and displayed correctly according the formula for 1st Product')
    ...
    KeywordUtil.logInfo(Total.toString() + ' : Scheme Total Amount calculated and displayed correctly according the formula for 2nd Product')
    ...
    Discount_value_Sheet = findTestData('Batch_Scheme_Inputs_01/B9_MTR_DV_2S_Buy_Excl_SL').getValue('Discount_value', Scheme_Index)

    Max_Limit = findTestData('Batch_Scheme_Inputs_01/B9_MTR_DV_2S_Buy_Excl_SL').getValue('Max_Limit', Scheme_Index)

    KeywordUtil.logInfo(Quantity_variations.get(i))

    if (((((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'Slab1_max_limit')) || (Quantity_variations.get(i) == 'Slab2_max_limit')) || (Quantity_variations.get(i) == 'Double_S2')) {
        'Positive flow Scheme Validation for Slab1 and Slab2 and Double slab'
        if (((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'Double_S2')) {
            ...
            Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/B9_MTR_DV_2S_Buy_Excl_SL').getValue('SchemeName', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed !')

            Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/B9_MTR_DV_2S_Buy_Excl_SL').getValue('Slab', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            'Scheme discount value for Product'
            GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/B9_MTR_DV_2S_Buy_Excl_SL').getValue('SKU_Name', Scheme_Index)
            ...
            KeywordUtil.logInfo(SchemeAmt_2 + ' : Scheme discount- Value is displayed properly for Product_2 !')

            if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) {
                Mobile.verifyEqual(Double.parseDouble(SchemeAmt_2), Double.parseDouble(Discount_value_Sheet), FailureHandling.STOP_ON_FAILURE)
            ...
            Mobile.tap(findTestObject('Mobile/SummaryScreen/SummaryScreen-info_closeBtn'), 0, FailureHandling.OPTIONAL)
        } else if ((Quantity_variations.get(i) == 'Slab1_max_limit') || (Quantity_variations.get(i) == 'Slab2_max_limit')) {
            'Scheme should be apply & scheme name should be display since it is positive flow'
            ...
            Mobile.verifyEqual(Double.parseDouble(SchemeAmt_2), Double.parseDouble(Discountvalue), FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.tap(findTestObject('Mobile/SummaryScreen/Click Order-PRINT ORDER btn'), 0) //Mobile.tap(findTestObject('Mobile/Common/Btn_Next'), 0)
    } else if ((Quantity_variations.get(i) == 'Less than slab1') || (Quantity_variations.get(i) == 'Excluded_product')) {
        'Scheme will not apply since it is negative scenario and Exclude Product'
