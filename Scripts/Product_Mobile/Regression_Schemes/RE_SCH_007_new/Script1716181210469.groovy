...
String sheet_name = '7_OTP_MS_Free_QTY(In_AND_Cond)'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
	...
	//slab_1
	'Slab_1'
	GlobalVariable.ProductName = findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('SKU_Name', Scheme_Index)
	...
	Slab_1_Min_Qty = findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Piece_quantity', Scheme_Index)
	...
	if ((Quantity_variations.get(i) == 'First slab') || (Quantity_variations.get(i) == 'Second slab')) {
		...
		Mobile.verifyMatch(SchemeName, findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

		Mobile.takeScreenshot()

		KeywordUtil.logInfo(SchemeName + ' : Slab Scheme Name correctly applied !')

		Mobile.verifyMatch(SchemeDesc, findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
		...
		GlobalVariable.Scheme_Free_SKU = findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_Product1', Scheme_Index)
		...
		Mobile.verifyMatch(FreeSKU_Qty_Slab, findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

		Mobile.takeScreenshot()

		KeywordUtil.logInfo(FreeSKU_Qty_Slab + ' : Free Product Case Qty correctly displayed !')

		Expecetd_FreeSKU_MinQty_Slab1 = ('Min:' + findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_quantity', Scheme_Index))
		...
		Expecetd_FreeSKU_MaxQty_Slab1 = ('Max:' + findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_quantity', Scheme_Index))
		...
		if (Quantity_variations.get(i) == 'Second slab') {
			'View screeen scheme validations for Slab2'
			GlobalVariable.Scheme_Free_SKU = findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_Product2', Scheme_Index)

			Mobile.waitForElementPresent(findTestObject('Mobile/OrderInvoice/Scheme/SchemeViewScreen_FreeSKU'), 5)

			FreeProduct2_Slab = Mobile.getText(findTestObject('Mobile/OrderInvoice/Scheme/SchemeViewScreen_FreeSKU'), 5)

			FreeSKU2_Qty_Slab = Mobile.getText(findTestObject('Mobile/OrderInvoice/Scheme/SchemeViewScreen_FreeSKU_Qty(Global)'), 5)

			FreeSKU2_MinQty_Slab = Mobile.getText(findTestObject('Mobile/OrderInvoice/Scheme/SchemeViewScreen_FreeSKU_MinQty(Global)'), 5)

			FreeSKU2_MaxQty_Slab = Mobile.getText(findTestObject('Mobile/OrderInvoice/Scheme/SchemeViewScreen_FreeSKU_MaxQty(Global)'), 5)

			Mobile.verifyMatch(FreeProduct2_Slab, GlobalVariable.Scheme_Free_SKU, false, FailureHandling.STOP_ON_FAILURE)

			Mobile.takeScreenshot()

			KeywordUtil.logInfo(FreeProduct2_Slab + ' :Slab  Free product correctly displayed !')

			Mobile.verifyMatch(FreeSKU2_Qty_Slab, findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

			Mobile.takeScreenshot()

			KeywordUtil.logInfo(FreeSKU2_Qty_Slab + ' : Free Product Case Qty correctly displayed !')

			Expecetd_FreeSKU_MinQty_Slab1 = ('Min:' + findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_quantity', Scheme_Index))

			Mobile.verifyMatch(FreeSKU2_MinQty_Slab.replaceAll('\\s', ''), Expecetd_FreeSKU_MinQty_Slab1, false, FailureHandling.STOP_ON_FAILURE)

			Mobile.takeScreenshot()

			KeywordUtil.logInfo(FreeSKU2_MinQty_Slab + ' : Free Product Min Case Qty correctly displayed !')

			Expecetd_FreeSKU_MaxQty_Slab1 = ('Max:' + findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_quantity', Scheme_Index))

			Mobile.verifyMatch(FreeSKU2_MaxQty_Slab.replaceAll('\\s', ''), Expecetd_FreeSKU_MaxQty_Slab1, false, FailureHandling.STOP_ON_FAILURE)

			Mobile.takeScreenshot()

			KeywordUtil.logInfo(FreeSKU2_MaxQty_Slab + ' : Free Product Max Piece Qty correctly displayed !')
		}
		...
		Mobile.verifyMatch(Slab1_FreeSku_PieceQty, findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
		...
		if (Quantity_variations.get(i) == 'Second slab') {
			'Summary screen scheme validations for Slab2 Free Product'
			GlobalVariable.Scheme_Free_SKU = findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_Product2', Scheme_Index)
			...
			KeywordUtil.logInfo(Slab1_FreeSku2 + ' : Free Product correctly displayed in Summary Screen !')

			Mobile.verifyMatch(Slab1_FreeSku2_PieceQty, findTestData('Mobile Input Data/7_OTP_MS_Free_QTY(In_AND_Cond)').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

			Mobile.takeScreenshot()

			KeywordUtil.logInfo(Slab1_FreeSku2_PieceQty + ' : Free Product Piece Qty correctly displayed in Summary Screen !')
		}
	...
	} else if (Quantity_variations.get(i) == 'Less than actual') {
