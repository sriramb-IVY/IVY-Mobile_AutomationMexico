...
String sheet_name = '6_B7_MTS_DV_Slab_1_BUY_EXCL'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/6_B7_MTS_DV_Slab_1_BUY_EXCL').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/6_B7_MTS_DV_Slab_1_BUY_EXCL').getValue('Excluded_SKU_Name_1', Scheme_Index)
    ...
    Min_Qty_1 = findTestData('Batch_Scheme_Inputs_01/6_B7_MTS_DV_Slab_1_BUY_EXCL').getValue('Piece_quantity', Scheme_Index)
    ...
    //Sku2
    'Buy_SKU_2'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/6_B7_MTS_DV_Slab_1_BUY_EXCL').getValue('SKU_Name_2', Scheme_Index)
    ...
    if (((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Double_Slab1')) || (Quantity_variations.get(i) == 'Excluded Product')) {
        'Positive flow Scheme Validation for Slab1'
        Mobile.tap(findTestObject('Mobile/OrderInvoice/Qty_Field'), 0)

        Min_Qty_2 = findTestData('Batch_Scheme_Inputs_01/6_B7_MTS_DV_Slab_1_BUY_EXCL').getValue('Piece_quantity', Scheme_Index)
        ...
        Discount_value_Sheet = findTestData('Batch_Scheme_Inputs_01/6_B7_MTS_DV_Slab_1_BUY_EXCL').getValue('Discount_Value', Scheme_Index)
        ...
        if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Double_Slab1')) {
            ...
            Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/6_B7_MTS_DV_Slab_1_BUY_EXCL').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed !')

            Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/6_B7_MTS_DV_Slab_1_BUY_EXCL').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            KeywordUtil.logInfo(SchemeAmt_1 + ' : Scheme discount Value is displayed properly for Product_1 !')

            Discount_value_for_excluded_product_sheet = findTestData('Batch_Scheme_Inputs_01/6_B7_MTS_DV_Slab_1_BUY_EXCL').getValue('Discount_Value', 3)

            Mobile.verifyEqual(Double.parseDouble(SchemeAmt_1), Double.parseDouble(Discount_value_for_excluded_product_sheet), FailureHandling.STOP_ON_FAILURE)

            KeywordUtil.logInfo(SchemeAmt_1 + ' : Scheme discount Value is zero bcz it is Excluded Product !')
            ...
            'Scheme will not apply for Excluded Product'
