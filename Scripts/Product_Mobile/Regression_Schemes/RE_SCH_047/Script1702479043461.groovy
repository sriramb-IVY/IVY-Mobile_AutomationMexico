...
String sheet_name = 'B10_OTPR_MS_DV_Brand'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/B10_OTPR_MS_DV_Brand').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    if (((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'Slab achieved and Remaining Qty')) {
        ...
        'Slab_1'
        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_MS_DV_Brand').getValue('SKU_Name', Scheme_Index)
        ...
        Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_MS_DV_Brand').getValue('Piece_quantity', Scheme_Index)

        Piece_Quantity = (Double.parseDouble(Slab_1_Min_Qty) / Double.parseDouble(Actual_PiecePrice))

        piece = Piece_Quantity.round()

        KeywordUtil.logInfo(piece + 'Piece quantity')

        GlobalVariable.Qty = piece.toString()
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/B10_OTPR_MS_DV_Brand').getValue('SchemeName', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed !')

        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/B10_OTPR_MS_DV_Brand').getValue('Slab', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        if (Quantity_variations.get(i) == 'Slab achieved and Remaining Qty') {
            ...
            'OTPR  =Remaining Qty*(Get Product Min /Buy Product max)'
            Buy_Max = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_MS_DV_Brand').getValue('Buy_Max', Scheme_Index)

            Get_Min = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_MS_DV_Brand').getValue('Get_Product_Min', Scheme_Index)

            Discount_sheet = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_MS_DV_Brand').getValue('Discount_value', Scheme_Index)

            Rem_quanty = (Total - Double.parseDouble(Buy_Max))

            KeywordUtil.logInfo(Rem_quanty + ' :Calculated Remaining qty!')

            Discount = (Double.parseDouble(Get_Min) / Double.parseDouble(Buy_Max))

            DiscountValue = (Rem_quanty * Discount)

            Discount_Value = DiscountValue.round()

            KeywordUtil.logInfo(Discount_Value + ' :Calculated Discount value!')

            Calculated_Discountvalue = (Discount_Value + Double.parseDouble(Discount_sheet))

            Mobile.verifyEqual(Double.parseDouble(Discountvalue), Calculated_Discountvalue, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()
        } else {
            Discount_value_Sheet = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_MS_DV_Brand').getValue('Discount_value', Scheme_Index)
        ...
        WebUI.callTestCase(findTestCase('Product_Mobile/Common/OrderPopUp(EditOrder)'), [:], FailureHandling.STOP_ON_FAILURE //        Mobile.delay(2)
            ) //
        //        Mobile.tap(findTestObject('Mobile/Common/Icon_Back'), 0)
        //
        //        Mobile.delay(2)
