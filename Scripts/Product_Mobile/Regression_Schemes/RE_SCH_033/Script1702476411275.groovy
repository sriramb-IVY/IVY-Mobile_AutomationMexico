...
String sheet_name = '19_B9_MTR_DV_MS_Get(Excl)_SL'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Excluded_SKU_Name_1', Scheme_Index)
    ...
    Min_Qty_1 = findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Piece_quantity', Scheme_Index)
    ...
    //Sku2
    'Buy_SKU_2'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('SKU_Name_1', Scheme_Index)
    ...
    Min_Qty_2 = findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Piece_quantity', Scheme_Index)
    ...
    //Sku3
    'Buy_SKU_3'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('SKU_Name_2', Scheme_Index)
    ...
    Min_Qty_3 = findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Piece_quantity', Scheme_Index)
    ...
    Discount_value_Sheet = findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Discount_Value', Scheme_Index)

    Max_Limit = findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Max_Limit', Scheme_Index)

    if ((((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'More than Slab1 Max Limit')) || (Quantity_variations.get(i) == 'More than Slab2 Max Limit')) {
        ...
        Discountvalue = Mobile.getText(findTestObject('Mobile/OrderInvoice/Scheme/Resource_1/Discout_Value_Top'), 0)

        Discountvalue_actual = Mobile.getText(findTestObject('Object Repository/Mobile/OrderInvoice/Scheme/DiscountValue'), 0)

        if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) {
            'Scheme Name Verification'
            Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeDesc + ' : Slab Description correctly displayed !')

            'Scheme Disc Value Verification'
        ...
        } else if ((Quantity_variations.get(i) == 'More than Slab1 Max Limit') || (Quantity_variations.get(i) == 'More than Slab2 Max Limit')) {
            'Scheme Name Verification'
            Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed for exclude Get Product!')
            ...
            KeywordUtil.logInfo(Limit_text.toString() + ' : Max limit value displaying text')
            ...
            KeywordUtil.logInfo(SchemeDesc + ' : Slab Description correctly displayed for exclude Get Product!')

            'Scheme Disc Value Verification'
            Mobile.verifyNotEqual(Double.parseDouble(Discountvalue), Double.parseDouble(Discount_value_Sheet), FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(Discountvalue + ' : Limit value is displaying as Discount Value because Slab exceeds the limit !')

            'Scheme Disc Value Verification With limit value'
            Mobile.verifyEqual(Double.parseDouble(Discountvalue), Double.parseDouble(Max_Limit), FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(Discountvalue_actual + ' :Limit value is displaying as Discount Value because Slab exceeds the limit !')
        }
        ...
        'Scheme discount value for Product_1'
        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/19_B9_MTR_DV_MS_Get(Excl)_SL').getValue('Excluded_SKU_Name_1', Scheme_Index)
        ...
        KeywordUtil.logInfo(SchemeAmt_1 + ' : Scheme Value is displayed properly for Product_1 !')
        ...
        KeywordUtil.logInfo(SchemeAmt_2 + ' : Scheme Value is displayed properly for Product_1 !')
        ...
        if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) {
            ...
            KeywordUtil.logInfo(SchemeAmt + ' : Discount Value expected and actual are equal!')
        } else if ((Quantity_variations.get(i) == 'More than Slab1 Max Limit') || (Quantity_variations.get(i) == 'More than Slab2 Max Limit')) {
            Mobile.verifyNotEqual(Double.parseDouble(SchemeAmt), Double.parseDouble(Discount_value_Sheet), FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeAmt + ' :Discount Value is not equal to the expected value because it Exceeds the Slab limit!')
        }
