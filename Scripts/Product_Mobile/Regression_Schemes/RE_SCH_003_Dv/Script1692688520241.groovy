...
String sheet_name = '3_MTS_SS_Discount Off'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input Data/3_MTS_SS_Discount Off').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    //slab_1
    'Slab_1'
    GlobalVariable.ProductName = findTestData('Mobile Input Data/3_MTS_SS_Discount Off').getValue('SKU_Name', Scheme_Index)
    ...
    Slab_1_Min_Qty = findTestData('Mobile Input Data/3_MTS_SS_Discount Off').getValue('Piece_quantity', Scheme_Index)
    ...
    if ((Quantity_variations.get(i) == 'single') || (Quantity_variations.get(i) == 'double')) {
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Mobile Input Data/3_MTS_SS_Discount Off').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed !')

        Mobile.verifyMatch(SchemeDesc, findTestData('Mobile Input Data/3_MTS_SS_Discount Off').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeDesc + ' : Slab Description correctly displayed !')

        Discount_value_Sheet = findTestData('Mobile Input Data/3_MTS_SS_Discount Off').getValue('Discount_Value', Scheme_Index)
