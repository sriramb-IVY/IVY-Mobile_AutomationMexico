...
String sheet_name = '5_MTS_MS_Discount Off'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    if (((Quantity_variations.get(i) == 'First slab') || (Quantity_variations.get(i) == 'Second slab')) || (Quantity_variations.get(i) == 'Both slab')) {
        'Positive flow Scheme Validation for Slab1'
        Mobile.tap(findTestObject('Mobile/Common/Icon_Search'), 4, FailureHandling.OPTIONAL)

        //slab_1
        'Slab_1'
        GlobalVariable.ProductName = findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('SKU_Name', Scheme_Index)
        ...
        Tot_value = findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('Sales_Value', Scheme_Index)
        ...
        if (Quantity_variations.get(i) == 'Both slab') {
            ...
            Mobile.verifyMatch(SchemeName1, findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            Mobile.verifyMatch(SchemeName2, findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            'Slab1 validation'
            Mobile.verifyMatch(SchemeDesc1, findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('Slab2', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            'Slab2 validation'
            Mobile.verifyMatch(SchemeDesc2, findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeDesc2 + ' : Slab Description correctly displayed !')

            Discount_value_Sheet = findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('Discount_Value', Scheme_Index)
            ...
            Mobile.verifyMatch(SchemeName, findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed !')

            Mobile.verifyMatch(SchemeDesc, findTestData('Mobile Input Data/5_MTS_MS_Discount Off').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        KeywordUtil.logInfo(Calculated_TotalAmt.toString() + ' : Slab Total amount in split screen displayed correctly !' // Negative Slab
            )
