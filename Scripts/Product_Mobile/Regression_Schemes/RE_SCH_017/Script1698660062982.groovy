...
String sheet_name = '17_B5_OTP_DP_Slab_1_Get_EXCL'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input Data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Mobile Input data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Excluded_SKU_Name_1', Scheme_Index)
    ...
    Min_Qty_1 = findTestData('Mobile Input data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Piece_quantity1', Scheme_Index)
    ...
    //Sku2
    'Buy_SKU_2'
    GlobalVariable.ProductName = findTestData('Mobile Input data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('SKU_Name_2', Scheme_Index)
    ...
    Min_Qty_2 = findTestData('Mobile Input data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Piece_quantity2', Scheme_Index)
    ...
    Discount_Perc_Sheet = findTestData('Mobile Input Data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Discount_percentage', Scheme_Index)

    if ((Quantity_variations.get(i) == 'First slab') || (Quantity_variations.get(i) == 'Excluded Product')) {
        ...
        if (Quantity_variations.get(i) == 'First slab') {
            'Scheme Name Verification'
            Mobile.verifyMatch(SchemeName, findTestData('Mobile Input Data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            Mobile.verifyMatch(SchemeDesc, findTestData('Mobile Input Data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            Mobile.delay(3)
        } else if (Quantity_variations.get(i) == 'Excluded Product') {
            'Scheme Name Verification'
            Mobile.verifyMatch(SchemeName, findTestData('Mobile Input Data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            KeywordUtil.logInfo(SchemeDesc + ' :Scheme Description correctly displayed for exclude Get Product!')

            'Scheme Discperc Verification'
            Mobile.verifyNotEqual(Double.parseDouble(DiscountPerc), Double.parseDouble(Discount_Perc_Sheet), FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(DiscountPerc + ' :Discount percentage is zero because Scheme will not apply for Get Exclude Product !')
        ...
        'Scheme discount Percentage for Product_1'
        GlobalVariable.ProductName = findTestData('Mobile Input data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Excluded_SKU_Name_1', Scheme_Index)
        ...
        'Scheme discount Percentage for Product_2'
        GlobalVariable.ProductName = findTestData('Mobile Input data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('SKU_Name_2', Scheme_Index)
        ...
        Discount_Perc_Sheet = findTestData('Mobile Input Data/17_B5_OTP_DP_Slab_1_Get_EXCL').getValue('Discount_percentage', 1)

        'scheme amount calculation in info screen'
        Calculated_Sch_Amt = ((Total_2 * Double.parseDouble(Discount_Perc_Sheet)) / 100)

        KeywordUtil.logInfo(Calculated_Sch_Amt.toString())

        Mobile.verifyEqual(SchemeAmt, Calculated_Sch_Amt, FailureHandling.STOP_ON_FAILURE)
