...
String sheet_name = '2_MTS_SS_Free_QTY'
...
ArrayList<String> Free_Product = CustomKeywords.'poi.Automation.GetAllDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'Free_Product')

//Mobile.tap(findTestObject('Mobile/Common/Btn_Menu'), 10)

WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    GlobalVariable.Scheme_Free_SKU = Free_Product.get(i)
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('SKU_Name', Scheme_Index)
    ...
    Slab_1_Min_Qty = findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('Piece_quantity', Scheme_Index)
    ...
    if ((Quantity_variations.get(i) == 'single') || (Quantity_variations.get(i) == 'double')) {
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.verifyMatch(SchemeDesc, findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        GlobalVariable.Scheme_Free_SKU = findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('Free_Product', Scheme_Index)
        ...
        Mobile.verifyMatch(FreeSKU_Qty_Slab, findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(FreeSKU_Qty_Slab + ' : Free Product Piece Qty correctly displayed !')

        Expecetd_FreeSKU_MinQty_Slab1 = ('Min:' + findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('Free_quantity', Scheme_Index))
        ...
        Expecetd_FreeSKU_MaxQty_Slab1 = ('Max:' + findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('Free_quantity', Scheme_Index))
        ...
        Mobile.verifyMatch(Slab1_FreeSku_PieceQty, findTestData('Mobile Input Data/2_MTS_SS_Free_QTY').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
