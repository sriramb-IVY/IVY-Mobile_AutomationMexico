...
String sheet_name = '9_B4_DP_OTP_AND'
...
ArrayList<String> SKU_Name_1 = CustomKeywords.'poi.Automation.GetAllDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'SKU_Name_1')

ArrayList<String> SKU_Name_2 = CustomKeywords.'poi.Automation.GetAllDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'SKU_Name_2')



WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input Data/9_B4_DP_OTP_AND').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    if (Quantity_variations.get(i) == 'First slab') {
        'Positive flow Scheme Validation for both Slab1'
        Mobile.tap(findTestObject('Mobile/Common/Icon_Search'), 4, FailureHandling.OPTIONAL)

        //sku_1
        'Slab_1'
        GlobalVariable.ProductName = findTestData('Mobile Input Data/9_B4_DP_OTP_AND').getValue('SKU_Name_1', Scheme_Index)
        ...
        'Entering piece qty for product1'
        Mobile.tap(findTestObject('Mobile/OrderInvoice/Qty_Field'), 0)

        Slab_1_Min_Qty = findTestData('Mobile Input Data/9_B4_DP_OTP_AND').getValue('Piece_quantity', Scheme_Index)
        ...
        Product1_Total = (Double.parseDouble(GlobalVariable.Qty) * Piece_Price)

        Mobile.verifyEqual(Double.parseDouble(SKU_TOTAL), Product1_Total, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(Product1_Total.toString() + ' : Product1 Total Amount calculated and displayed correctly according the formula.')

        //Sku_2
        'Slab_1'
        GlobalVariable.ProductName = findTestData('Mobile Input Data/9_B4_DP_OTP_AND').getValue('SKU_Name_2', Scheme_Index)
        ...
        'Entering piece qty for product2'
        Mobile.tap(findTestObject('Mobile/OrderInvoice/Qty_Field'), 0)
        ...
        Product2_Total = (Double.parseDouble(GlobalVariable.Qty) * Piece_Price)

        Mobile.verifyEqual(Double.parseDouble(SKU_TOTAL), Product2_Total, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(Product2_Total.toString() + ' : Product2 Total Amount calculated and displayed correctly according the formula.')

        Total = (Product1_Total + Product2_Total)

        KeywordUtil.logInfo(Total.toString() + ' : Sum of both the products Total Amount calculated and displayed correctly according the formula.')
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Mobile Input Data/9_B4_DP_OTP_AND').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeName + ' :Scheme Name correctly applied !')

        Mobile.verifyMatch(SchemeDesc, findTestData('Mobile Input Data/9_B4_DP_OTP_AND').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeDesc + ' :Scheme Description correctly displayed !')

        Discount_Perc_Sheet = findTestData('Mobile Input Data/9_B4_DP_OTP_AND').getValue('Discount_percentage', Scheme_Index)
