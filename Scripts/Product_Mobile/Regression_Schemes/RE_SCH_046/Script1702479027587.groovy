...
String sheet_name = 'B10_OTPR_SS_DV'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/B10_OTPR_SS_DV').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab achieved and Remaining Qty')) {
        ...
        'Slab_1'
        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_SS_DV').getValue('SKU_Name', Scheme_Index)
        ...
        Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_SS_DV').getValue('Piece_quantity', Scheme_Index)
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/B10_OTPR_SS_DV').getValue('SchemeName', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed !')

        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/B10_OTPR_SS_DV').getValue('Slab', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        if (Quantity_variations.get(i) == 'Slab achieved and Remaining Qty') {
            ...
            Buy_Max = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_SS_DV').getValue('Buy_Max', Scheme_Index)

            Get_Min = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_SS_DV').getValue('Get_Product_Min', Scheme_Index)

            Remin_Qty = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_SS_DV').getValue('Piece_quantity', Scheme_Index)

            Rem_quanty = (Double.parseDouble(Remin_Qty) - Double.parseDouble(Buy_Max))

            Discount = (Double.parseDouble(Get_Min) / Double.parseDouble(Buy_Max))

            Discount_Value = (Rem_quanty * Discount)

            KeywordUtil.logInfo(Discount_Value + ' :Calculated Discount value!')

            Calculated_Discountvalue = (Discount_Value + Double.parseDouble(Get_Min))

            Mobile.verifyEqual(Double.parseDouble(Discountvalue), Calculated_Discountvalue, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()
        } else {
            Discount_value_Sheet = findTestData('Batch_Scheme_Inputs_01/B10_OTPR_SS_DV').getValue('Discount_value', Scheme_Index)
