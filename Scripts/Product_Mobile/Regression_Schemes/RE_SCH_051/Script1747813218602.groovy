...
String sheet_name = '5_B7_MTS_DV_SS_(BUY_EXCL)CAT'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/5_B7_MTS_DV_SS_(BUY_EXCL)CAT').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/5_B7_MTS_DV_SS_(BUY_EXCL)CAT').getValue('SKU_Name_1', Scheme_Index)
    ...
    Min_Qty_1 = findTestData('Batch_Scheme_Inputs_01/5_B7_MTS_DV_SS_(BUY_EXCL)CAT').getValue('Piece_quantity', Scheme_Index)
    ...
    //Sku2
    'Buy_SKU_2'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/5_B7_MTS_DV_SS_(BUY_EXCL)CAT').getValue('SKU_Name_2', Scheme_Index)
    ...
    if (((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Double_Slab1')) || (Quantity_variations.get(i) == 'Excluded Product')) {
        ...
        Min_Qty_2 = findTestData('Batch_Scheme_Inputs_01/5_B7_MTS_DV_SS_(BUY_EXCL)CAT').getValue('Piece_quantity', Scheme_Index)
        ...
        Discount_value_Sheet = findTestData('Batch_Scheme_Inputs_01/5_B7_MTS_DV_SS_(BUY_EXCL)CAT').getValue('Discount_Value', Scheme_Index)
        ...
        if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Double_Slab1')) {
            ...
            Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/5_B7_MTS_DV_SS_(BUY_EXCL)CAT').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed !')

            Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/5_B7_MTS_DV_SS_(BUY_EXCL)CAT').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            SchemeAmt = (SchemeAmountt_1 + SchemeAmountt_2)

            KeywordUtil.logInfo('SchemeAmount' + SchemeAmt)

            KeywordUtil.logInfo(SchemeAmt + ' : Total Scheme discount- Value for both the Products !')
