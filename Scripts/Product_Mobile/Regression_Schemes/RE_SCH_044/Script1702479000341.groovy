...
String sheet_name = 'B9_MTR_FP_MS_GET(EXCL)'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Excluded_SKU_Name_1', Scheme_Index)
    ...
    Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Piece_quantity', Scheme_Index)
    ...
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('SKU_Name_1', Scheme_Index)
    ...
    Actual_BasePrice_2 = Mobile.getText(findTestObject('Mobile/OrderInvoice/ProductDetails/BasePrice_Value_Field'), 0)

    Actual_PiecePrice_2 = Mobile.getText(findTestObject('Mobile/OrderInvoice/ProductDetails/PiecePrice_Value_Field'), 0)

    Piece_Price_2 = Double.parseDouble(Actual_PiecePrice_2)

    KeywordUtil.logInfo('Price of the product' + Piece_Price_2)

    Actual_CaseSize_2 = Mobile.getText(findTestObject('Mobile/OrderInvoice/ProductDetails/CaseSize_Value_Field'), 0)

    Mobile.tap(findTestObject('Mobile/OrderInvoice/Qty_Field'), 0)

    Slab_1_Min_Qty_2 = findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Piece_quantity', Scheme_Index)

    println(GlobalVariable.Qty = Slab_1_Min_Qty_2)
    ...
    SKU_TOTAL_2 = Mobile.getText(findTestObject('Mobile/OrderInvoice/OrderInvoiceScrn-SkuTotal'), 0)

    KeywordUtil.logInfo('SKU Total Value ' + SKU_TOTAL_2)

    'total value calculation of second product'
    Total_2 = (Double.parseDouble(GlobalVariable.Qty) * Piece_Price_2)

    'verification of total amount'
    Mobile.verifyEqual(Double.parseDouble(SKU_TOTAL_2), Total_2, FailureHandling.STOP_ON_FAILURE)

    Mobile.takeScreenshot()

    KeywordUtil.logInfo(Total_2.toString() + ' : Scheme Total Amount calculated and displayed correctly according the formula.')

    overall_Total = (Total_2 + Total)

    KeywordUtil.logInfo(overall_Total.toString() + ' : Scheme overall Total Amount calculated and displayed correctly according the formula.')

    Mobile.tap(findTestObject('Mobile/Common/Btn_Next'), 0)

    WebUI.callTestCase(findTestCase('Product_Mobile/Common/MustSell_Alert'), [:], FailureHandling.STOP_ON_FAILURE)

    if (((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'Double_Slab2')) {
        'Positive flow Scheme Validation for Double slab'
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Slab', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Free_Product', Scheme_Index)
        ...
        Mobile.verifyMatch(FreeSKU_Qty_Slab, findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Free_Get_Quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(FreeSKU_Qty_Slab + ' : Free Product Piece Qty correctly displayed !')

        Expecetd_FreeSKU_MinQty_Slab1 = ('Min:' + findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Free_Get_Quantity', Scheme_Index))
        ...
        Expecetd_FreeSKU_MaxQty_Slab1 = ('Max:' + findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Free_Get_Quantity', Scheme_Index))
        ...
        Mobile.verifyMatch(Slab1_FreeSku_PieceQty, findTestData('Batch_Scheme_Inputs_01/B9_MTR_FP_MS_GET(EXCL)').getValue('Free_Get_Quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.verifyEqual(Double.parseDouble(OrderAmt), overall_Total, FailureHandling.STOP_ON_FAILURE)
