...
String sheet_name = '18_B9_MTR_FP_AD_DIS_SS(Variant)'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Quantity_variations.size(); i++) {
    Scheme_Index = (i + 1)

    Mobile.tap(findTestObject('Mobile/Common/Icon_Search'), 4, FailureHandling.OPTIONAL)

    if (((Quantity_variations.get(i) == 'SLAB1') || (Quantity_variations.get(i) == 'Double First Slab')) || (Quantity_variations.get(i) == 'Less than slab1')) {
        'Buy_SKU_1'
        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Buy_Group1_P1', Scheme_Index)
        ...
        Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Min_quantity', Scheme_Index)
        ...
        'Buy_SKU_2'
        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Buy_Group1_P2', Scheme_Index)
        ...
        Slab_2_Min_Qty = findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Min_quantity', Scheme_Index)
        ...
        total_qty_entered = (Double.parseDouble(Slab_1_Min_Qty) + Double.parseDouble(Slab_2_Min_Qty))
    ...
    } else if (Quantity_variations.get(i) == 'Variant Count') {
        'Buy_SKU_1'
        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Buy_Group1_P1', Scheme_Index)
        ...
        GlobalVariable.TotalValue = Total
    ...
    if ((Quantity_variations.get(i) == 'SLAB1') || (Quantity_variations.get(i) == 'Double First Slab')) {
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeDesc + ' : Scheme Description correctly displayed !')

        'Additional disc verification'
        ...
        add_disc = findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Additional_Discount', Scheme_Index)

        additional_disc = Double.parseDouble(add_disc)

        double cal1 = GlobalVariable.Total_qty * additional_disc

        KeywordUtil.logInfo('Calculation1 of additional disc ' + cal1)

        cal2 = (cal1 * GlobalVariable.Total)

        KeywordUtil.logInfo('Calculation2 of 1st product ' + cal2)

        scheme_disc_for_one_product = (cal2 / GlobalVariable.Total)

        scheme_disc_product1 = scheme_disc_for_one_product.round(2)

        KeywordUtil.logInfo('scheme_disc_product1 ' + scheme_disc_product1)

        Mobile.verifyEqual(cal1, Additional_Discount_Amt, FailureHandling.STOP_ON_FAILURE)

        KeywordUtil.logInfo(cal1 + ' : Additional discount is displayed properly !')
        ...
        GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Free_Product', Scheme_Index)
        ...
        Mobile.verifyMatch(FreeSKU_Qty_Slab, findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(FreeSKU_Qty_Slab + ' : Free Product Piece Qty correctly displayed !')

        Expecetd_FreeSKU_MinQty_Slab1 = ('Min:' + findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Free_quantity', Scheme_Index))
        ...
        Expecetd_FreeSKU_MaxQty_Slab1 = ('Max:' + findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Free_quantity', Scheme_Index))
        ...
        Mobile.verifyMatch(Slab1_FreeSku_PieceQty, findTestData('Batch_Scheme_Inputs_01/18_B9_MTR_FP_AD_DIS_SS(Variant)').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
    ...
    } else if ((Quantity_variations.get(i) == 'Less than slab1') || (Quantity_variations.get(i) == 'Variant Count')) {
        ...
        if (Quantity_variations.get(i) == 'Less than slab1') {
            OrderAmt = Mobile.getText(findTestObject('Mobile/SummaryScreen/SplitScreen-OrderAmt'), 0)
            ...
            Mobile.verifyEqual(Double.parseDouble(OrderAmt), GlobalVariable.TotalValue, FailureHandling.STOP_ON_FAILURE)
