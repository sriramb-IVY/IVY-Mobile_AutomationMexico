...
String sheet_name = '23_B10_OTPR_SS_DV_VALUE'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/23_B10_OTPR_SS_DV_VALUE').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    //slab_1
    'Slab_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/23_B10_OTPR_SS_DV_VALUE').getValue('SKU_Name', Scheme_Index)
    ...
    Tot_value = findTestData('Batch_Scheme_Inputs_01/23_B10_OTPR_SS_DV_VALUE').getValue('Sales_Value', Scheme_Index)
    ...
    if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'More than Slab1')) {
        Mobile.takeScreenshot()
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/23_B10_OTPR_SS_DV_VALUE').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed !')

        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/23_B10_OTPR_SS_DV_VALUE').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeDesc + ' : Slab Description correctly displayed !')

        Discount_value_Sheet = findTestData('Batch_Scheme_Inputs_01/23_B10_OTPR_SS_DV_VALUE').getValue('Discount_Value', Scheme_Index)
        ...
        if (Quantity_variations.get(i) == 'Slab1') {
            ...
            KeywordUtil.logInfo(Discountvalue + ' :Slab Discount Value correctly applied for Slab1 !')
        } else if (Quantity_variations.get(i) == 'More than Slab1') {
            ...
            KeywordUtil.logInfo(Discountvalue + ' :Slab Discount Value correctly applied for more than slab1 so it is not equal to Slab1 discount value!')
        }
        
        'OTPR calculation'
        if (Quantity_variations.get(i) == 'More than Slab1') {
            ...
            sales_value = findTestData('Batch_Scheme_Inputs_01/23_B10_OTPR_SS_DV_VALUE').getValue('Sales_Value', Scheme_Index)

            double Disvalue_Qty_min = Double.parseDouble(Discount_Value)

            double BuyMax = Double.parseDouble(Buy_Max)

            double tvalue = Double.parseDouble(sales_value)

            double Calculation1 = Disvalue_Qty_min / BuyMax

            KeywordUtil.logInfo('cal1:' + Calculation1)

            double Calculation2 = tvalue - BuyMax
            ...
            double remaining_slab = Remainingquantity + Disvalue_Qty_min

            KeywordUtil.logInfo('remaining_slab:' + remaining_slab)

            WebUI.verifyEqual(remaining_slab, Discountvalue)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(Discountvalue + ' :Calculated and displayed Discount Value correctly applied for More than Slab1 !')

            GlobalVariable.RemainingSlab = remaining_slab
        }
        ...
        if (Quantity_variations.get(i) == 'Slab1') {
            ...
            KeywordUtil.logInfo(SchemeAmt + ' : More than Slab1 Scheme amount displayed in split screen so it is not equal  !')

            WebUI.verifyEqual(GlobalVariable.RemainingSlab, Double.parseDouble(SchemeAmt), FailureHandling.STOP_ON_FAILURE)

            KeywordUtil.logInfo(SchemeAmt + ' : More than Slab1 Scheme amount displayed correctly as per the calculation in split screen !')
        }
