...
String sheet_name = '15_B5_OTP_DV_BUY(EXCL)'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input data/15_B5_OTP_DV_BUY(EXCL)').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Mobile Input data/15_B5_OTP_DV_BUY(EXCL)').getValue('Excluded_SKU_Name_1', Scheme_Index)
    ...
    Min_Qty_1 = findTestData('Mobile Input data/15_B5_OTP_DV_BUY(EXCL)').getValue('Piece_quantity1', Scheme_Index)
    ...
    //Sku2
    'Buy_SKU_2'
    GlobalVariable.ProductName = findTestData('Mobile Input data/15_B5_OTP_DV_BUY(EXCL)').getValue('SKU_Name_2', Scheme_Index)
    ...
    Min_Qty_2 = findTestData('Mobile Input data/15_B5_OTP_DV_BUY(EXCL)').getValue('Piece_quantity2', Scheme_Index)
    ...
    Discount_value_Sheet = findTestData('Mobile Input data/15_B5_OTP_DV_BUY(EXCL)').getValue('Discount_Value', Scheme_Index)
    ...
    if ((Quantity_variations.get(i) == 'First slab') || (Quantity_variations.get(i) == 'Excluded Product')) {
        ...
        if (Quantity_variations.get(i) == 'First slab') {
            ...
            Mobile.verifyMatch(SchemeName, findTestData('Mobile Input data/15_B5_OTP_DV_BUY(EXCL)').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeName + ' : Scheme Name correctly displayed !')

            Mobile.verifyMatch(SchemeDesc, findTestData('Mobile Input data/15_B5_OTP_DV_BUY(EXCL)').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            Discount_value_for_excluded_product_sheet = findTestData('Mobile Input data/15_B5_OTP_DV_BUY(EXCL)').getValue('Discount_Value', 2)
            ...
            KeywordUtil.logInfo(SchemeAmt_1 + ' : Scheme discount- Value is zero for Excluded Product!')
