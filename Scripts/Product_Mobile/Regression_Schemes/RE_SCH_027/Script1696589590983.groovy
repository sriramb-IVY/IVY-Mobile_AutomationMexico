...
String sheet_name = '11_B8_MTR_FP_1S_QTY'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    //slab_1
    'Slab_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('SKU_Name', Scheme_Index)
    ...
    Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('Piece_quantity', Scheme_Index)
    ...
    if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Double_Slab1')) {
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('Free_Product', Scheme_Index)
        ...
        Mobile.verifyMatch(FreeSKU_Qty_Slab, findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(FreeSKU_Qty_Slab + ' : Free Product Piece Qty correctly displayed !')

        Expecetd_FreeSKU_MinQty_Slab1 = ('Min:' + findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('Free_quantity', Scheme_Index))
        ...
        Expecetd_FreeSKU_MaxQty_Slab1 = ('Max:' + findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('Free_quantity', Scheme_Index))
        ...
        Mobile.verifyMatch(Slab1_FreeSku_PieceQty, findTestData('Batch_Scheme_Inputs_01/11_B8_MTR_FP_1S_QTY').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
