...
String sheet_name = '15_B9_MTR_MS_APPR_FP_Get(ANY)'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    if (((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'Double_Slab2')) {
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Expecetd_FreeSKU_MinQty_Slab = ('Min:' + findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Free_Get_Quantity', Scheme_Index))
        ...
        Expecetd_FreeSKU_MaxQty_Slab = ('Max:' + findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Free_Get_Quantity', Scheme_Index))
        ...
        if (Integer.parseInt(FreeSKU_1_Qty_Slab) != 0) {
            ...
            Mobile.verifyMatch(FreeSKU_1_Qty_Slab, findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Free_Get_Quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Get_Product02', Scheme_Index)

            FreeSKU_2_Qty_Slab = Mobile.getText(findTestObject('Mobile/OrderInvoice/Scheme/SchemeViewScreen_FreeSKU_Qty(Global)'), 5)

            Mobile.verifyMatch(FreeSKU_2_Qty_Slab, findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Free_Get_Quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        GlobalVariable.Qty = findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Free_Get_Quantity', Scheme_Index)

        Mobile.verifyMatch(Slab_FreeSku_PieceQty, findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Free_Get_Quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Slab_Qty = findTestData('Batch_Scheme_Inputs_01/15_B9_MTR_MS_APPR_FP_Get(ANY)').getValue('Free_Get_Quantity', Scheme_Index)
        ...
        Buy_Products_OrderValue = Buy_Product_Total

        Buy_Product_Splited_Scheme_Amt = ((Scheme_Applied_Get_Product_Total_Amount * Buy_Product_Total) / Buy_Products_OrderValue)

        KeywordUtil.logInfo(Buy_Product_Splited_Scheme_Amt.toString() + ' : Buy_Product_Splited_Scheme_Amt')

        Scheme_Amt_for_Buy_Product = Mobile.getText(findTestObject('Mobile/SummaryScreen/SplitScreen_SCHEME Amt'), 0)

        Scheme_Amt_for_Buy_Product = Scheme_Amt_for_Buy_Product.replaceAll('- ', '')

        KeywordUtil.logInfo(Scheme_Amt_for_Buy_Product)

        Mobile.verifyEqual(Double.parseDouble(Scheme_Amt_for_Buy_Product), Buy_Product_Splited_Scheme_Amt, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        'Validation for Total'
        Buy_Product_TotalAmt = Mobile.getText(findTestObject('Mobile/SummaryScreen/SliptScreen_TotalAmt'), 0)

        Calculated_Buy_Product_TotalAmt = (Buy_Product_Total - Buy_Product_Splited_Scheme_Amt)

        Mobile.verifyEqual(Double.parseDouble(Buy_Product_TotalAmt), Calculated_Buy_Product_TotalAmt, FailureHandling.STOP_ON_FAILURE)
        ...
        Buy_Products_OrderValue = (Buy_Product_Total + Scheme_Applied_Get_Product_Total_Amount)
        ...
        WebUI.callTestCase(findTestCase('Product_Mobile/Regression_Schemes/Call Test Cases/RE_SCH_031_CT_1'), [:], FailureHandling.STOP_ON_FAILURE)
    }
}
