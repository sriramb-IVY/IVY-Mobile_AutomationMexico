...
String sheet_name = '10_B4_SS_FP(VALUE)_OTP'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    if (Quantity_variations.get(i) == 'First slab') {
        ...
        //slab_1
        'Slab_1'
        GlobalVariable.ProductName = findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('SKU_Name', Scheme_Index)
        ...
        Tot_value = findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('Sales_Value', Scheme_Index)
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeName + ' : Slab Scheme Name correctly applied !')

        Mobile.verifyMatch(SchemeDesc, findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        GlobalVariable.Scheme_Free_SKU = findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('Free_Product', Scheme_Index)
        ...
        Mobile.verifyMatch(FreeSKU_Qty_Slab, findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(FreeSKU_Qty_Slab + ' : Free Product Case Qty correctly displayed !')

        Expecetd_FreeSKU_MinQty_Slab1 = ('Min:' + findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('Free_quantity', Scheme_Index))
        ...
        Expecetd_FreeSKU_MaxQty_Slab1 = ('Max:' + findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('Free_quantity', Scheme_Index))
        ...
        Mobile.verifyMatch(Slab1_FreeSku_PieceQty, findTestData('Mobile Input Data/10_B4_SS_FP(VALUE)_OTP').getValue('Free_quantity', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        //        WebUI.callTestCase(findTestCase('Product_Mobile/Common/OrderPopUp(EditOrder)'), [:], FailureHandling.STOP_ON_FAILURE)
			...
			Mobile.tap(findTestObject('Mobile/Store_Actvy/Menu-OrderInvoice'), 0, FailureHandling.STOP_ON_FAILURE)
					...
					WebUI.callTestCase(findTestCase('Product_Mobile/Common/OrderPopUp(EditOrder)'), [:], FailureHandling.STOP_ON_FAILURE)
    } else if (Quantity_variations.get(i) == 'First slab') {
        ...
        int Quantity = (Sales_Value / Piece_Price) - 1
