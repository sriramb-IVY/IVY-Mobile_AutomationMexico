...
//GlobalVariable.index = 31
String sheet_name = 'Scheme_CriteriaMapping'

String file_name = 'Mobile Input data'

ArrayList<String> Retailer_name = CustomKeywords.'poi.Automation.GetAllDataRow_Flag.getAllDataRow'(file_name, sheet_name, 
    'Retailer')

ArrayList<String> Sales_person = CustomKeywords.'poi.Automation.GetAllDataRow_Flag.getAllDataRow'(file_name, sheet_name, 
    'Sales_person')

ArrayList<String> Location_name_mapping = CustomKeywords.'poi.Automation.GetAllDataRow_Flag.getAllDataRow'(file_name, 
    sheet_name, 'Location_name_mapping')
...
Location_name_mapped = Location_name_mapping.get(GlobalVariable.index)
...
WebUI.sendKeys(findTestObject('Web Part/Scheme Request/Retailer_mapping/name_input'), Retailer_name.get(1))

GlobalVariable.label = Retailer_name.get(1)

retailername = WebUI.getText(findTestObject('Web Part/Scheme Request/Retailer_mapping/Data_Select'))
...
WebUI.click(findTestObject('Web Part/Scheme Request/Retailer_mapping/branch_mapping_label_btn'))
...
WebUI.click(findTestObject('Web Part/Scheme Request/Retailer_mapping/select_tab'))
...
ExcelKeywords.setValueToCellByIndex(sheet1, 1 + GlobalVariable.index, 3, channel)

ExcelKeywords.setValueToCellByIndex(sheet1, 1 + GlobalVariable.index, 4, GlobalVariable.branch_name)

ExcelKeywords.setValueToCellByIndex(sheet1, 1 + GlobalVariable.index, 5, retailername)

ExcelKeywords.setValueToCellByIndex(sheet1, 1 + GlobalVariable.index, 6, attribute)
