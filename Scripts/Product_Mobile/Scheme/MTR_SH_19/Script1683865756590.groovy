...
Calculated_Sku_Total = ((Double.parseDouble(Actual_CaseSize) * Double.parseDouble(GlobalVariable.Qty)) * Double.parseDouble(Actual_PiecePrice))

Mobile.verifyEqual(Double.parseDouble(SKU_TOTAL), Calculated_Sku_Total, FailureHandling.STOP_ON_FAILURE)

Mobile.takeScreenshot()

KeywordUtil.logInfo(Calculated_Sku_Total.toString() + ' :  Sku Total Amount calculated and displayed correctly according the formula.')
...
Mobile.verifyEqual(Double.parseDouble(OrderAmt), Calculated_Sku_Total, FailureHandling.STOP_ON_FAILURE)
