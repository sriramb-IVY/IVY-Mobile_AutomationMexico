...
ArrayList<String> BuyProduct2 = CustomKeywords.'poi.Automation.GetAllDataRow_Flag.getAllDataRow'(file_name, sheet_name, 
    'BuyProduct2')
...
for (int a = 0; a < BuyProduct1.size(); a++) {
    if (BuyProduct1.get(a) != 'null') {
        ...
        WebUI.scrollToElement(findTestObject('Web Part/Product_Hierarchy/Piece_MRP_Value'), 0)

        MRP_amt = WebUI.getText(findTestObject('Web Part/Product_Hierarchy/Piece_MRP_Value'))
		
		GlobalVariable.MRP = MRP_amt.replace('$ ', '')
