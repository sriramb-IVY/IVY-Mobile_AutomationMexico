...
GlobalVariable.SO_Number = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('OrderNumber', 2)
...
Mobile.callTestCase(findTestCase('Product_Mobile/Common/DB'), [('Query') : 'select * from appdata_sales_invoice_scheme_detail where sisd_sih_id=\'' + GlobalVariable.label + '\'', ('columnNames') : ['SISD_Percentage','SISD_Value','SISD_AS_Id','SISD_APH_Id']], FailureHandling.STOP_ON_FAILURE)



String Percentage = GlobalVariable.data[0]

//percentage
DiscountPerc = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('DiscountPerc', 2)

Mobile.verifyEqual(Percentage, Double.parseDouble(DiscountPerc), FailureHandling.STOP_ON_FAILURE)

KeywordUtil.logInfo('Scheme Discount percentage from the mobile should be properly inserted into database')

//Scheme Amt
Strng SISD_Value = GlobalVariable.data[1]

SchemeAmt = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('SchemeAmt', 2)

Mobile.verifyEqual(SISD_Value, Double.parseDouble(SchemeAmt), FailureHandling.STOP_ON_FAILURE)
...
SchemeName = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('SchemeName', 2)
...
ProductName = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('ProductName', 2)
...
GlobalVariable.ProductName = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('ProductName', 2)
...
WebUI.verifyMatch(Price, findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('Price', 2), false, FailureHandling.STOP_ON_FAILURE)
...
WebUI.verifyMatch(Qty, findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('Qty', 2), false, FailureHandling.STOP_ON_FAILURE)
...
TotalAmt = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('Total_Amt', 2)
...
GrossAmt = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('Gross_Amt', 2)
...
Expected_SchemeName = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('SchemeName', 2)
