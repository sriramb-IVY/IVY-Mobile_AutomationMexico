...
'SH_38 Credit Scheme DB validation'
...
//2rd 
queryString5 = (('Select top 2 * from AppData_Sales_Invoice_Scheme_Detail_Credit where SISDC_SIH_Id =\'' + GlobalVariable.label) + 
'\' ')
...
while (recordSet5.next()) {
    ...
    KeywordUtil.logInfo(SISDC_Value.toString())

    Values.add(SISDC_Value)
}
...
Mobile.verifyEqual(sum, GlobalVariable.FreeSkuTotal, FailureHandling.STOP_ON_FAILURE)
...
GlobalVariable.DiscountAmt = findTestData('Web Input Data/Sales_Order_Invoice_View').getValue('DiscountAmt', CreditNote)

Mobile.verifyEqual(sum2.round(1),  GlobalVariable.DiscountAmt , FailureHandling.STOP_ON_FAILURE)
