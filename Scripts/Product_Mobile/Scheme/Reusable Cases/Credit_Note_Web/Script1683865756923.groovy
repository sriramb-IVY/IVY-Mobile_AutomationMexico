...
GlobalVariable.label = findTestData('Mobile Input Data/RetailerTypeCreditNote_Scheme').getValue('Distributor', 1)
...
GlobalVariable.label = findTestData('Mobile Input Data/RetailerTypeCreditNote_Scheme').getValue('Channel_Name', 1)
...
for (int i = 0; i < Retailer.size(); i++) {
    if (Retailer.get(i) != 'null') {
        ...
        WebUI.scrollToElement(findTestObject('Web Part/RetailerAttribute/SchemeApplyOn'), 100)

        SchemeApplyNo = WebUI.getAttribute(findTestObject('Web Part/RetailerAttribute/SchemeApplyOn'), 'value')
