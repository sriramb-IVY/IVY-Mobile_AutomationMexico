...
for (int i = 0; i <( Retailer.size()); i++) {
	...
	Mapped_Retailer = findTestData('Mobile Input Data/Fixed_Variable_Scheme').getValue('MappedRetailer', 13)
    ...
    if (Retailer.get(i) != 'null' && (Retailer.get(i) == Mapped_Retailer || Retailer.get(i) == UnMapped_Retailer)) {
        ...
        if (SchemeAppyType.contains(RetailerApplyType) == true) {
            KeywordUtil.logInfo('For this Retailer "SchemeApplyType" and "RetailerApplyType" was Equal, hence we can validate scheme !')
        } else {
        ...
        if (Mobile.verifyElementVisible(findTestObject('Mobile/OrderInvoice/Scheme/SchemeName'), 
			...
			Web_Addition_dis_Amt = findTestData('Mobile Input Data/Fixed_Variable_Scheme').getValue('Additional_Dis_Amt', Scheme_Index)
