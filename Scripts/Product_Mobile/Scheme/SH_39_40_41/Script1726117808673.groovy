...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    SchemeAppyNo = findTestData('Mobile Input Data/RetailerTypeCreditNote_Scheme').getValue('SchemeAppyNo', Scheme_Index + i)
    ...
    if (Mobile.verifyMatch(SchemeAppyNo, Expected_SchemeAppyNo, false, FailureHandling.STOP_ON_FAILURE)) {
        ...
        //WebUI.callTestCase(findTestCase('VBL_Mobile/Re-Usables/Select_Retailer and Going_OrderInvoice_Screen'), [('RetailerName') : findTestData('Mobile Input Data/RetailerTypeCreditNote_Scheme').getValue('Retailer', Scheme_Index + i)], FailureHandling.STOP_ON_FAILURE)
        ...
        GlobalVariable.keypadValue = findTestData('Mobile Input Data/RetailerTypeCreditNote_Scheme').getValue('Slab_1_Min_Qty', Scheme_Index)
        ...
        //        Slab1_FreeSku_Tax_Amt = Mobile.getText(findTestObject('XXXXXXXXXXXX/Mobile Part/Store_Actvy/SummaryScreen/Summary_ProductBasedTotalAmt(Global)'), 
        //            5)
        //
        ...
        Calculated_TotalAmt = (SKU_TOTAL + Double.parseDouble(CGST))
