...
if (Mobile.verifyElementExist(findTestObject('Mobile/AndroidDevice/RecentApp_CloseAll_Btn'), 1, FailureHandling.OPTIONAL)) {
    Mobile.delay(1)

    Mobile.tap(findTestObject('Mobile/AndroidDevice/RecentApp_CloseAll_Btn'), 0)
}

Mobile.switchToPortrait()

MobileDriverFactory.getDriver().activateApp(GlobalVariable.appPackage)
...
/* else {
    ...
    for (int i = 0; i <= 5; i++) {
        ...
        if (Mobile.verifyElementExist(findTestObject('Mobile/AndroidDevice/RecentApp_CloseAll_Btn'), 2, FailureHandling.OPTIONAL)) {
            Mobile.delay(3)
...
if (CloudDeviceName.contains('Pixel')) {
    ...
    for (i = 0; i <= 5; i++) {
        if (Mobile.verifyElementExist(findTestObject('Mobile/AndroidDevice/IvyCpg Logo in Google'), 2, FailureHandling.OPTIONAL)) {
            Mobile.tap(findTestObject('Mobile/AndroidDevice/IvyCpg Logo in Google'), 0)
    ...
    Mobile.tap(findTestObject('Mobile/AndroidDevice/Google_Search_Field'), 0)

    Mobile.setText(findTestObject('Mobile/AndroidDevice/Google_Search_Field'), 'Ivy', 0, FailureHandling.STOP_ON_FAILURE)
	Mobile.delay(3)
    if (Mobile.verifyElementExist(findTestObject('Mobile/AndroidDevice/IvyCpg Logo in Google'), 2, FailureHandling.OPTIONAL)) {
		Mobile.delay(2)
		Mobile.tap(findTestObject('Mobile/AndroidDevice/IvyCpg Logo in Google'), 0)
