...
String sheet_name = 'Empties_Scheme_OTP_DP'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Salable_SKU_Name1', Scheme_Index)
    ...
    Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Buy_Quantity', Scheme_Index)

    println(GlobalVariable.Qty = Slab_1_Min_Qty)

    GlobalVariable.label = GlobalVariable.Qty
    ...
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Salable_SKU_Name2', Scheme_Index)
    ...
    Slab_1_Min_Qty_1 = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Buy_Quantity1', Scheme_Index)
    ...
    Total = ((Double.parseDouble(GlobalVariable.label) * Piece_Price) + (Double.parseDouble(Slab_1_Min_Qty_1) * Piece_Price1))
    ...
    if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) {
		...
		Mobile.verifyElementVisible(findTestObject('Mobile/Seller_2/Stock and Order/Empty_Product_Details_Title'), 15)
				...
				GlobalVariable.label = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Empty_SKU_Name1', Scheme_Index)
				...
				'Empty SKU 2'
				GlobalVariable.label = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Empty_SKU_Name2', Scheme_Index)
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeDesc + ' : Scheme Description correctly displayed !')

        Discount_Perc = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('DiscountPercentage', Scheme_Index)

        Mobile.verifyEqual(Double.parseDouble(Discountpercentage), Double.parseDouble(Discount_Perc), FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(Discountpercentage + ' :Discount Value correctly applied !')

        'Prodcut 1 discount Calculation'
        Perc = (Double.parseDouble(Discount_Perc) / 100)
        ...
        'Product1'
        GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Empty_SKU_Name1', Scheme_Index)

        Mobile.verifyElementVisible(findTestObject('Mobile/SummaryScreen/Summary_FreeSkuTitle(Global)'), 5)

        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Empty_SKU_Name1', Scheme_Index)
        ...
        'Product2'
        GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Empty_SKU_Name2', Scheme_Index)

        Mobile.verifyElementVisible(findTestObject('Mobile/SummaryScreen/Summary_FreeSkuTitle(Global)'), 5)

        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DP').getValue('Empty_SKU_Name2', Scheme_Index)
