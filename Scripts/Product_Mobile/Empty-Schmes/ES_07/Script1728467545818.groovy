...
String sheet_name = 'Empties_Scheme_MTS_DV'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('Salable_SKU_Name1', Scheme_Index)
    ...
    Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('Buy_Quantity', Scheme_Index)
    ...
    KeywordUtil.logInfo('SKU1 Total Value ' + SKU_TOTAL)
    ...
    if ((((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'Slab1&2')) || (Quantity_variations.get(i) == 'Double of Slab2')) {
        ...
        GlobalVariable.label = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('Empty_SKU_Name1', Scheme_Index)
        ...
        Return_Cal = (Double.parseDouble(Empty_liable_qty_1) * Double.parseDouble(Empty_price_1))
        ...
        if (((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'Double of Slab2')) {
            Mobile.comment('Validation for MTS Discount Value scheme ')
            ...
            Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeDesc + ' : Scheme Description correctly displayed !')

            Discount = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('DiscountValue', Scheme_Index)

            Mobile.verifyEqual(Double.parseDouble(Discountvalue), Double.parseDouble(Discount), FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(Discountvalue + ' :Discount Amount correctly applied !')

            GlobalVariable.Waiveoff_amt = Discountvalue
        } else if (Quantity_variations.get(i) == 'Slab1&2') {
            Mobile.comment('Validation for MTS slab1 and Slab2 discount amount')
            ...
            'Slab Description Verification'
            Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('Slab_Description_MTS', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeDesc + ' : Scheme Slab2 Description correctly displayed !')

            Discount_Slab2 = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('DiscountValue1', Scheme_Index)

            Mobile.verifyEqual(Double.parseDouble(Discountvalue1), Double.parseDouble(Discount_Slab2), FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(Discountvalue1 + ' :Slab2 Discount amount correctly applied  !')
            ...
            Discount_Slab1 = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('DiscountValue', Scheme_Index)

            Mobile.verifyEqual(Double.parseDouble(Discountvalue2), Double.parseDouble(Discount_Slab1), FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.comment('Validation for Discount amount applied in empty product')

        GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('Empty_SKU_Name1', Scheme_Index)

        Mobile.verifyElementVisible(findTestObject('Mobile/SummaryScreen/Summary_FreeSkuTitle(Global)'), 5)

        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_DV').getValue('Empty_SKU_Name1', Scheme_Index)
