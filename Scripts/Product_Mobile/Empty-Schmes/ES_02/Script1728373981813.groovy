...
String sheet_name = 'Empties_Scheme_OTP_DV'
...
ArrayList<String> DiscountValue = CustomKeywords.'poi.Automation.GetAll_Batch_Scheme_Inputs_01_Flag.getAllDataRow'(file_name, sheet_name, 'DiscountValue')

Mobile.tap(findTestObject('Mobile/Common/Btn_Menu'), 10)

WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Salable_SKU_Name1', Scheme_Index)
    ...
    Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Buy_Quantity', Scheme_Index)
    ...
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Salable_SKU_Name2', Scheme_Index)
    ...
    Slab_1_Min_Qty_1 = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Buy_Quantity1', Scheme_Index)
    ...
    if (Quantity_variations.get(i) == 'Slab1') {
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeDesc + ' : Scheme Description correctly displayed !')

        Discount_amt = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('DiscountValue', Scheme_Index)

        Mobile.verifyEqual(Double.parseDouble(Discountvalue), Double.parseDouble(Discount_amt), FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(Discountvalue + ' :Discount Value correctly applied !')

        Mobile.tap(findTestObject('Mobile/Common/Btn_Next'), 0)
		
		Mobile.verifyElementVisible(findTestObject('Mobile/Seller_2/Stock and Order/Empty_Product_Details_Title'), 15)
		
				GlobalVariable.label = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Empty_SKU_Name1', Scheme_Index)
				...
				'Empty SKU 2'
				GlobalVariable.label = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Empty_SKU_Name2', Scheme_Index)
        ...
        GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Empty_SKU_Name1', Scheme_Index)

        Mobile.verifyElementVisible(findTestObject('Mobile/SummaryScreen/Summary_FreeSkuTitle(Global)'), 5)

        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_DV').getValue('Empty_SKU_Name1', Scheme_Index)
