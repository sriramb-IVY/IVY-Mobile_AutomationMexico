...
String sheet_name = 'Empties_Scheme_OTP_Discount_Eve'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('Salable_SKU_Name1', Scheme_Index)
    ...
    Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('Buy_Quantity', Scheme_Index)
    ...
    if ((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Discount Every')) {
        ...
        Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(SchemeDesc + ' : Scheme Description correctly displayed !')

        Qty_Every = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('Every_qty', Scheme_Index)

        Dis_amt = (Double.parseDouble(GlobalVariable.Qty) / Double.parseDouble(Qty_Every))

        Discount_amt = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('DiscountValue', Scheme_Index)

        Disc_Cal = (Dis_amt * Double.parseDouble(Discount_amt))

        KeywordUtil.logInfo(Disc_Cal + ' :Discount amount for Every scheme!')

        Mobile.verifyEqual(Double.parseDouble(DiscountAmount), Disc_Cal, FailureHandling.STOP_ON_FAILURE)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(DiscountAmount + ' :Discount amount correctly applied for Discount Every scheme !')

        Mobile.tap(findTestObject('Mobile/Common/Btn_Next'), 0)
		
		Mobile.verifyElementVisible(findTestObject('Mobile/Seller_2/Stock and Order/Empty_Product_Details_Title'), 15)
		
				GlobalVariable.label = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('Empty_SKU_Name1', Scheme_Index)
        ...
        GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('Empty_SKU_Name1', Scheme_Index)

        Mobile.verifyElementVisible(findTestObject('Mobile/SummaryScreen/Summary_FreeSkuTitle(Global)'), 5)

        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_OTP_Discount_Eve').getValue('Empty_SKU_Name1', Scheme_Index)
        ...
        KeywordUtil.logInfo(SchemeAmt_1 + ' : Scheme  Amount is displayed properly for the Product !')

        Mobile.verifyEqual(Double.parseDouble(SchemeAmt_1), Disc_Cal, FailureHandling.STOP_ON_FAILURE)
