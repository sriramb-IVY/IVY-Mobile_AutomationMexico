...
String sheet_name = 'Empties_Scheme_MTS_SL_EM_FP'
...
WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Retailer', 1)], FailureHandling.STOP_ON_FAILURE)
...
for (int i = 0; i < Retailer.size(); i++) {
    ...
    'Buy_SKU_1'
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Salable_SKU_Name1', Scheme_Index)
    ...
    Slab_1_Min_Qty = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Buy_Quantity', Scheme_Index)
    ...
    GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Salable_SKU_Name2', Scheme_Index)
    ...
    Slab_1_Min_Qty_1 = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Buy_Quantity1', Scheme_Index)
    ...
    if ((((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'Slab1&2')) || (Quantity_variations.get(i) == 'Double of Slab2')) {
        ...
        GlobalVariable.label = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Empty_SKU_Name1', Scheme_Index)
        ...
        'Empty SKU 2'
        GlobalVariable.label = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Empty_SKU_Name2', Scheme_Index)
        ...
        if (((Quantity_variations.get(i) == 'Slab1') || (Quantity_variations.get(i) == 'Slab2')) || (Quantity_variations.get(i) == 'Double of Slab2')) {
            ...
            Freeproduct_qty = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Free_Product_Get_Qty1', Scheme_Index)

            Free_Product_Amount = (Double.parseDouble(Freeproduct_qty) * Double.parseDouble(Empty_price_1))
            ...
            Mobile.verifyMatch(SchemeName, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Scheme_Name', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)
            ...
            Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Slab_Description', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeDesc + ' : Scheme Description correctly displayed !')

            Mobile.verifyEqual(Double.parseDouble(Discountvalue), GlobalVariable.Waiveoff_amt, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(Discountvalue + ' :Free product amount correctly applied in discount column !')
        } else if (Quantity_variations.get(i) == 'Slab1&2') {
            Mobile.comment('Calculation for Free Product waive off')

            Freeproduct_qty1 = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Free_Product_Get_Qty1', Scheme_Index)

            Freeproduct_qty2 = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Free_Product_Get_Qty_Slab2', Scheme_Index)

            Freeproduct_qty = (Double.parseDouble(Freeproduct_qty1) + Double.parseDouble(Freeproduct_qty2))

            Free_Product_Amount = (Freeproduct_qty * Double.parseDouble(Empty_price_1))

            KeywordUtil.logInfo(Free_Product_Amount + ' : Free Product Amount')

            GlobalVariable.Waiveoff_amt = Free_Product_Amount

            Slab1_WaiveoffAmount = (Double.parseDouble(Freeproduct_qty1) * Double.parseDouble(Empty_price_1))

            Slab2_WaiveoffAmount = (Double.parseDouble(Freeproduct_qty2) * Double.parseDouble(Empty_price_1))
            ...
            'Slab Description Verification'
            Mobile.verifyMatch(SchemeDesc, findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Slab_Description_MTS', Scheme_Index), false, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(SchemeDesc + ' : Scheme Slab2 Description correctly displayed !')

            Mobile.verifyEqual(Double.parseDouble(Discountvalue), Slab2_WaiveoffAmount, FailureHandling.STOP_ON_FAILURE)

            Mobile.takeScreenshot()

            KeywordUtil.logInfo(Discountvalue + ' :Slab2 Free product amount correctly applied in discount value !')
            ...
            Discountvalue2 = Mobile.getText(findTestObject('Mobile/OrderInvoice/Scheme/DiscountValue_2'), 0)
            ...
            Mobile.verifyEqual(Double.parseDouble(Discountvalue2), Slab1_WaiveoffAmount, FailureHandling.STOP_ON_FAILURE)
        ...
        Mobile.comment('Validation for Free product amount applied in slable product')

        GlobalVariable.Scheme_Free_SKU = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Empty_SKU_Name1', Scheme_Index)

        Mobile.verifyElementVisible(findTestObject('Mobile/SummaryScreen/Summary_FreeSkuTitle(Global)'), 5)

        GlobalVariable.ProductName = findTestData('Batch_Scheme_Inputs_01/Empties_Scheme_MTS_SL_EM_FP').getValue('Empty_SKU_Name1', Scheme_Index)
        ...
        KeywordUtil.logInfo(SchemeAmt_1 + ' : Scheme Waive off Amount is displayed properly for Product !')
        ...
        Mobile.verifyEqual(Double.parseDouble(SchemeAmt), GlobalVariable.Waiveoff_amt, FailureHandling.STOP_ON_FAILURE)
