...
for (int i = 0; i < Warehouse.size(); i++) {
	if (Warehouse.get(i) != 'NULL') {
		...
		KeywordUtil.logInfo('Manual data added in Stock allocation Successfully!!')
	}
}
...
queryString = 'select top 1 * from AppData_Van_Load_Header where VLH_AUH_Id = 1585 order by 1 desc'
...
if (recordSet.next()) {
  ...
  KeywordUtil.logInfo('Van Load Reference no  :  ' + GlobalVariable.VanLoad_No )
	...
	Assert.fail()
	KeywordUtil.logInfo('Data not available in DB')
}
...
String Sheetname = 'Empties_VanLoad'
