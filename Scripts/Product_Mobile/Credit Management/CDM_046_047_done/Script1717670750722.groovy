...
CustomKeywords.'credit_management.fetch_query.delete_invoice'(findTestData('Mobile Input Data/update_credit_warn_1').getValue('Retailer_Name',8))
...
Retailer_Name = findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Credit_Retailer', 4)

CustomKeywords.'credit_management.Limit_period_bill_increase.decrease_limits'(Retailer_Name)

Warn_Retailer_Name = findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Credit_Retailer', 1)

CustomKeywords.'credit_management.Limit_period_bill_increase.warn_Creditbill'(Warn_Retailer_Name)

String sheet_name = 'Credit_Bill_Inc_after_invoice'
...
//Product_Name = findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Product_Name', 1)

//Case_qty = findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('CaseQty', 1)
...
for (int i = 0; i < Credit_Validation.size(); i++) {
    ...
    WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue(
    ...
    //slab_1
    'Slab_1'
    GlobalVariable.ProductName = findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Product_Name', 
        Scheme_Index)
	...
	Slab_1_Min_Qty = findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Piece_quantity', Scheme_Index)
   ...
   // if ((((i == 0) || (i == 2)) || (i == 3)) || (i == 5)) {
	if ((Credit_Validation.get(i) == 'Less_Credit') || (Credit_Validation.get(i) == 'Credit_Bill(increased and submitted)') || (Credit_Validation.get(i) == 'Less_Warn') || (Credit_Validation.get(i) == 'Warn_Bill(increased and submitted)')) {
        ...
        Mobile.verifyMatch(Invoice_submitted_popup, findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Alert_Popup', 
                Scheme_Index), false)
    ...
    } else if (Credit_Validation.get(i) == 'Warn_Bill(Soft)') {
        ...
        Mobile.verifyMatch(Warning_message_popup, findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Alert_Popup', 
                Scheme_Index), false)
        ...
        Retailer_Name = findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Credit_Retailer', Scheme_Index)

        CustomKeywords.'credit_management.Limit_period_bill_increase.warn_Creditbill'(Retailer_Name)
    ...
    } else if (Credit_Validation.get(i) == 'Credit_Bill(Hard)') {
		
       GlobalVariable.Alert_message = findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Alert_Popup', Scheme_Index)
        ...
        Mobile.verifyMatch(bill_exceed_alert_popup, findTestData('Mobile Input Data/Credit_Bill_Inc_after_invoice').getValue('Alert_Popup', 
                Scheme_Index), false)
        ...
        CustomKeywords.'credit_management.Limit_period_bill_increase.increase_Creditbill'(Retailer_Name)
