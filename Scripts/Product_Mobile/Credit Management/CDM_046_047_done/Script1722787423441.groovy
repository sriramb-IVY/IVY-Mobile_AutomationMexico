...
Warn_Retailer_Name = findTestData('Mobile Input Data/Credit_limit_Inc_after_invoice').getValue('Credit_Retailer', 4)
...
for (int i = 0; i < Credit_Validation.size(); i++) {
	...
	Slab_1_Min_Qty = findTestData('Mobile Input Data/Credit_limit_Inc_after_invoice').getValue('Piece_quantity', Scheme_Index)
	...
	if ((Credit_Validation.get(i) == 'Credit_limit(increased and submitted)') || (Credit_Validation.get(i) == 'Warn_Limit(Increased and submitted)') ) {
        ...
        Mobile.verifyMatch(Invoice_submitted_popup, findTestData('Mobile Input Data/Credit_limit_Inc_after_invoice').getValue('Alert_Popup', 
                Scheme_Index), false)
        ...
        Mobile.verifyMatch(Warning_message_popup, findTestData('Mobile Input Data/Credit_limit_Inc_after_invoice').getValue('Alert_Popup', 
                Scheme_Index), false)
        ...
        CustomKeywords.'credit_management.Limit_period_bill_increase.increase_warncredit_limit'(Retailer_Name)
       ...
       GlobalVariable.Alert_message = findTestData('Mobile Input Data/Credit_limit_Inc_after_invoice').getValue('Alert_Popup', Scheme_Index)
       ...
       limit_exceed_alert_popup = Mobile.getText(findTestObject('Object Repository/Mobile/Credit Management/Alert_Popup_global'), 
        ...
        Mobile.verifyMatch(limit_exceed_alert_popup, findTestData('Mobile Input Data/Credit_limit_Inc_after_invoice').getValue('Alert_Popup', 
                Scheme_Index), false)
