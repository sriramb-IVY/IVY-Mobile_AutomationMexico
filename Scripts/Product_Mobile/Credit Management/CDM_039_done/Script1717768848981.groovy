...
CustomKeywords.'credit_management.fetch_query.query'(CustomKeywords.'credit_management.fetch_query.LIMITS_overdue'(findTestData(
			...
			'Balance', 2), findTestData('Mobile Input Data/CreditManagement').getValue('overdue', 4)))

//Mobile.startApplication(GlobalVariable.APKFile, false)

String sheet_name = 'Credit_Bill_Collection'
...
for (int i = 0; i < Credit_Validation.size(); i++) {
	...
	WebUI.callTestCase(findTestCase('Product_Mobile/Common/TradeCoverage(SelectRetailer)'), [('RetailerName') : findTestData('Mobile Input Data/Credit_Bill_Collection').getValue(
	...
	GlobalVariable.ProductName = findTestData('Mobile Input Data/Credit_Bill_Collection').getValue('Product_Name',
		Scheme_Index)
	...
	Slab_1_Min_Qty = findTestData('Mobile Input Data/Credit_Bill_Collection').getValue('Piece_quantity', Scheme_Index)
	...
	if ((Credit_Validation.get(i) == 'Less_Credit') || (Credit_Validation.get(i) == 'Before_Collection') || (Credit_Validation.get(i) == 'After_Collection')) {
		
		Submit_Popup = findTestData('Mobile Input Data/Credit_Bill_Collection').getValue('Alert_Popup', Scheme_Index)
	...
	} else if (Credit_Validation.get(i) == 'Exceed_Credit') {
		
		GlobalVariable.Alert_message = findTestData('Mobile Input Data/Credit_Bill_Collection').getValue('Alert_Popup', Scheme_Index)
