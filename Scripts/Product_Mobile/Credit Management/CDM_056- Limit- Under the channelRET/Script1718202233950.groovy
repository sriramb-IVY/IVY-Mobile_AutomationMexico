...
for (int i = 0; i<Channel_Retailer.size(); i++) {
	...
	GlobalVariable.Qty = Quantity.toString()
		...
		///need to check/////	
			...
			{
				Order_Alert_Text = findTestData('Mobile Input Data/Period_Bal_Combination').getValue('Order Exceed', 2)
	...
	{	
		...
		/*AppiumDriver driver = MobileDriverFactory.getDriver()
				...
				Warning_Limit_message_popup = driver.findElementByXPath('//android.widget.Toast[1]').getText()
		
				KeywordUtil.logInfo(Warning_Limit_message_popup)
	
				Mobile.verifyMatch(Warning_Limit_message_popup,Limit.get(i), false)*/
	...
	WebUI.callTestCase(findTestCase('Product_Mobile/Credit Management/Call Test Case/CDM_COLL_CT_01'), [:], FailureHandling.STOP_ON_FAILURE)
