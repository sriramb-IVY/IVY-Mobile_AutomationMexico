...
'Order Rfield - 0 Hard Alert'
//
CustomKeywords.'credit_management.Admin_Configs_1.Set_Configs'('CREDIT01', '0')

//
'combination of period and order hard'
String sheet_name = 'Period_Hard_AT_Collection'
...
ArrayList<String> Order_Exceed = CustomKeywords.'poi.Automation.GetAllDataRow_Flag.getAllDataRow'(file_name, sheet_name, 
    'Order Exceed')
...
for (int i = 0; i < Channel_Retailer.size(); i++) {
    if (Retailer_Level.get(i) == 'Hard Period Alert') {
...
//    CustomKeywords.'credit_management.Stock_manual_load_1.Add_Stock'(Product1, Qty)
    ...
    Mobile.setText(findTestObject('Mobile/Common/Field_Search_EnterText'), Channel_Retailer.get(
            i), 5)
    ...
    if (Retailer_Level.get(i) == 'Soft period & Hard Order exceed') {
        ...
        Mobile.tap(findTestObject('Object Repository/Mobile/Credit Management/Credit Period_PROCEED_Btn'), 
        ...
        'Order Exceed hard alert'
        Mobile.verifyElementVisible(findTestObject('Object Repository/Mobile/Credit Management/Order_exceeds_CD_Bal'), 
            10)

        Order_Exceeds_Hard_Alert = Mobile.getText(findTestObject('Object Repository/Mobile/Credit Management/Order_exceeds_CD_Bal'), 
            0)

        Mobile.takeScreenshot()

        KeywordUtil.logInfo(Order_Exceeds_Hard_Alert)

        Mobile.verifyMatch(Order_Exceeds_Hard_Alert, Order_Exceed.get(i), false)
