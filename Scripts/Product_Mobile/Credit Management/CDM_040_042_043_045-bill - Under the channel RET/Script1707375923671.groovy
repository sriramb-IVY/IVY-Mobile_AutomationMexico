...
'combination of period and bill'

//Mobile.startApplication(GlobalVariable.APKFile, false)


String sheet_name = 'Period_Bill_Combination'
...
ArrayList<String> Bill_Exceed = CustomKeywords.'poi.Automation.GetAllDataRow_Flag.getAllDataRow'(file_name, sheet_name, 'Bill Exceed')
...
Mobile.tap(findTestObject('Mobile/Common/Btn_Menu'), 30)
	...
	Mobile.setText(findTestObject('Mobile/Common/Field_Search_EnterText'),findTestData('Mobile Input Data/Period_Bill_Combination').getValue('Channel_Retailer', 1), 10)
	
	Mobile.tap(findTestObject('Mobile/TradeCoverage/First_Retailer_Selection_From_List'), 15)
...
for (int i = 0; i<Channel_Retailer.size(); i++) {
	...
	Mobile.tap(findTestObject('Object Repository/Mobile/Credit Management/Credit Period_PROCEED_Btn'), 0, FailureHandling.STOP_ON_FAILURE)
	...
	Mobile.takeScreenshot()
		...
		///need to check///
		if(Retailer_Level.get(i) =='Bill Hard Alert')
			
	{
		
		Order_Alert_Text = findTestData('Mobile Input Data/Period_Bill_Combination').getValue('Order Exceed', 2)
	...
	'Credit Bill Warning Alert'

	//if(i==7)
		
		if(Retailer_Level.get(i) =='warn bill Alert')
			
	{
		
		Bill_Alert_Text = findTestData('Mobile Input Data/Period_Bill_Combination').getValue('Bill Exceed', 1)
		
		CustomKeywords.'credit_management.Alert_Popup_1.Warn_Alert'(Bill_Alert_Text)
		...
		Mobile.verifyMatch(Bill_Warning_message_popup,Bill_Exceed.get(i), false)*/
	}

	
		
	if(Retailer_Level.get(i) =='After the period limit'||Retailer_Level.get(i) =='warn bill Alert')
		{
		...
		if(Retailer_Level.get(i) =='Bill Hard Alert')		
	{
		...
		Mobile.verifyElementVisible(findTestObject('Object Repository/Mobile/Credit Management/Alert_Text'), 10)
				...
				String Limit_Hard_Alert_Text = Mobile.getText(findTestObject('Object Repository/Mobile/Credit Management/Alert_Text'),
						0)
					
				Mobile.takeScreenshot()
		
				KeywordUtil.logInfo(Limit_Hard_Alert_Text)
		
				Mobile.verifyMatch(Limit_Hard_Alert_Text, GlobalVariable.Alert, false)
		
				Mobile.tap(findTestObject('Mobile/Common/Btn_OK'), 0)
		
				Mobile.delay(3)
		
				GlobalVariable.Alert = Bill_Exceed.get(i)
				
		Alert_Text = findTestData('Mobile Input Data/Period_Bill_Combination').getValue('Bill Exceed', 2)
...
//	'Collecting invoice'
//
//			WebUI.callTestCase(findTestCase('Test Cases/Product_Mobile/Credit Management/Call Test Case/CDM_COLL_CT_01'), [:], FailureHandling.STOP_ON_FAILURE)
