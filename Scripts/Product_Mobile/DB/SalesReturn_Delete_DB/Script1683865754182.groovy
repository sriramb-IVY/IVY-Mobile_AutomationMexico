...
queryString = (('select  * from appdata_sales_return_header where srh_Date = \'' + DB_Currentdate) + '\'   and SRH_UserId=\'' + GlobalVariable.vanseller_user_id + '\' ')
...
while (recordSet.next()) {
	...
	String data = recordSet.getObject('SRH_date')
	
	println(data)


queryString01 = (('delete from appdata_sales_return_detail where srd_srh_id=\'' + GlobalVariable.label) + '\' and SRD_Created_By = \'' + GlobalVariable.vanseller_user_id + '\'')
...
queryString0 = (('delete from appdata_sales_return_reasons   where srr_srh_id =\'' + GlobalVariable.label) + '\' and SRR_Created_By = \'' + GlobalVariable.vanseller_user_id + '\' ')
...
queryString1 = (((('delete from appdata_sales_return_header where srh_Date = \'' + DB_Currentdate) + '\'  and SRH_Id = \'') +
GlobalVariable.label) + '\' and SRH_UserId=\'' + GlobalVariable.vanseller_user_id + '\' ')
