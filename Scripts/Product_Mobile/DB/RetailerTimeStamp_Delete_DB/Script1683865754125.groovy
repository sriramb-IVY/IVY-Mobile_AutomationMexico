...
queryString = (('select top 1 * from AppData_Retailer_Timestamp  where RT_Visit_Date = \'' + DB_Currentdate) + '\' and RT_User_Id=\'' + GlobalVariable.vanseller_user_id + '\'')
...
while (recordSet.next()) {
	String data1 = recordSet.getObject('RT_Id')

	KeywordUtil.logInfo(data1)

	GlobalVariable.label = data1
	
	String data = recordSet.getObject('RT_Visit_Date')
	
	println(data)


queryString01 = (('delete  from  AppData_Retailer_Timestamp_Detail  where RTD_Id =\'' + GlobalVariable.label) + '\' and RTD_Created_By = \'' + GlobalVariable.vanseller_user_id + '\'')
...
queryString0 = (('delete  from AppData_Retailer_Timestamp  where RT_Id = \'' + GlobalVariable.label) + '\'')
