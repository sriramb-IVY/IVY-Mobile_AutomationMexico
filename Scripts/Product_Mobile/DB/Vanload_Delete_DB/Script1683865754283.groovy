...
queryString = (('select * from AppData_Van_Load_Header where VLH_Date = \'' + DB_Currentdate) + '\'   and VLH_AUH_Id=\'' + GlobalVariable.vanseller_user_id + '\'')
...
while (recordSet.next()) {
	String data1 = recordSet.getObject('VLH_Id')

	KeywordUtil.logInfo(data1)

	GlobalVariable.label = data1
	
	String data2 = recordSet.getObject('VLH_Date')
	
	println(data2)	


queryString01 = (('delete from appdata_van_load_detail where VLD_VLH_Id =\'' + GlobalVariable.label) + '\'')
...
queryString0 = (('delete from AppData_Van_Load_Header where VLH_Id = \'' + GlobalVariable.label) + '\'   and VLH_AUH_Id=\'' + GlobalVariable.vanseller_user_id + '\'')
