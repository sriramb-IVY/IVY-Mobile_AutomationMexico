...
for (def index : (0..Nav.size() - 1)) {
	...
	if ((Nav[index]) == (Nav[previousIndex])) {
		...
		if (NavLinks.size() > 0) {
			...
			GlobalVariable.SAttributetext = "Collection Accept (Mobile)"
				...
				WebUI.scrollToElement(findTestObject('Surendran/IvySalesOrder/Page_IvyDMS/a_NavigationLink'),0)
	...
	if ((Nav[index]) == 'Receivables') {
		...
		WebUI.click(findTestObject('Surendran/IvySalesOrder/Page_IvyDMS/a_NavigationLink'))
	}
}
